...
/**
 * <p>
 * This class provides instrumentation of JTree objects. What this means is
 * that by associating a JTree with an instance of the Instrumenter class,
 * you can get features such as drag-and-drop, import/export XML, tree
 * structure rearranging and editing of node names pretty much for free. While
 * some of these features require you to do some adaption coding (particularly
 * if you're not 100% happy with the default behaviour), such adaption is
 * designed to be reasonably easy and we also provide means of disabling
 * features that you do not desire.
 * </p>
 *
 * <p>
 ...
 * </pre>
 * You now proceed to put your tree into your GUI and will immediately
 * reap the benefit of the instrumenter's many features. If you intend to
 * interact with the instrument in the future, you will want to keep a
 * reference to it. If not, you do not need to do so.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * To get the maximum usability with the least amount of fuss, it is
 * recommended that you use tree nodes of type DefaultMutableTreeNode. The
 * instrumenter expects this type for a variety of its features. It should
 * be evident in the API documentation what features that require this. If
 * you find any discrepancies, please let us know.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * Instances of Instrumenter can only be obtained by calling one of its
 * factory methods;
...
public class Instrumenter
{
   ...
   private Instrumenter()
   {
	...
	for (Enumeration e = ceListeners.elements() ; e.hasMoreElements() ;) {
	    ((DefaultTreeCellEditor)
	     m_tree.getCellEditor()).removeCellEditorListener((CellEditorListener)e.nextElement());
	}
   ...
   /**
    * Factory method for instrumenting a tree and returning
    * the instrument.
    *
    * @param tree Tree to instrument. It must already have been
    * constructed (i.e., null pointer not allowed).
    * @param full_instrumentation If true, will install all instrumentation
    ...
    * as parameter.
    * @param clear If true, will clear the contents of the tree and create a
    ...
    */
   public static Instrumenter createInstrument(
      JTree tree,
      ...
      boolean clear)
   {
      ...
      if (clear && retval.getNodeFactory()!=null)
         ((DefaultTreeModel)tree.getModel()).
            setRoot(retval.getNodeFactory().createNode(null));
      return retval;
   }

   /**
    * Convenience factory method instrumenting a tree while keeping the
    * tree's contents.
    *
    * @see #createInstrument(JTree,boolean,boolean)
    */
   ...
   /**
    * Convenience factory method to fully instrument a tree with all
    * available functionality, keeping its contents.
    *
    * @see #createInstrument(JTree,boolean,boolean)
    * @see #createInstrument(JTree,boolean
    */
   ...
   /**
    * <p>
    * This method tweaks a cell editor so that we can use it. Specifically,
    * we add a CellEditorListener to it so we can handle focus when editing
    * stops.
    * </p>
    *
    * <p>
    * NOTE that when the cell editor changes, we make no attempt to remove
    * the listener from the old cell editor. This is a potential source of
    * memory leaks.
    * </p>
    *
   ...
   protected void configureCellEditor()
   {
       CellEditorListener ce = new CellEditorListener()
      {
       ...
       ((DefaultTreeCellEditor)m_tree.getCellEditor()).addCellEditorListener(ce);
       ceListeners.add(ce);
   }

   /**
    * <p>
    * Will configure the tree to use the default Drag And Drop
    * implementation. This will work for many applications, and when it
    * does not is is generally easy to make it work. If Drag And Drop is
    * not wanted, however, it may make sense to disable it.
    * </p>
    * This method is called automatically if you choose
    * full instrumentation in the call to createInstrument.
    *
    * @param b If true, uses our default tree config. If false, does nothing.
    *
    * @see #createInstrument(JTree,boolean,boolean)
    */
   ...
   /**
    * <p>
    * Will configure the tree for optimal instrumented operation. If you
    * do not call this method (it is called automatically if you choose
    * full instrumentation in the call to createInstrument), take note of the
    * following;
    * <ul>
    ...
    * <li>Scrolling of the tree may not function as you wish unless you
    * call setAutoscrolls(true) on your tree</li>
    * </ul>
    * </p>
    *
    * @param b If true, uses our default tree config. If false, does nothing.
    ...
    * node factory has no particular expectations of the type of nodes or
    * their contents but creates DefaultMutableTreeNode objects with
    * DefaultNodeInfo contents when constructing new nodes.
    *
    * @param b If true, will use our default node factory. If false, does
    * nothing.
    *
    * @see #createInstrument(JTree,boolean,boolean)
    */
    ...
    * The default undo manager has not particular expectations of the
    * nodes or their contents.
    *
    * @param b If true, will call setUndoManager() with a new InfoUndoManager.
    ...
    * default shortcuts (i.e., Ctrl-Up, Ctrl-Down, Space) conflict
    * with and override
    * default Swing JTree shortcuts. The keyboard shortcuts will not
    * work properly unless you also install the default action map
    * (or the subset of it that is of interest to you).
    *
    * @param b If true, will install our keyboard shortcuts. If false, removes
   ...
   /**
    * Configures whether to use our default actions. While these should
    * not conflict with existing ones, you don't really need to install
    * them if you don't intend to use them (i.e., if you haven't installed
    * the default keyboard map).  The keyboard shortcuts will not
    * work properly unless you also install the default action map
    * (or the subset of it that is of interest to you).
    *
    * @param b If true, will install our actions. If false, removes
    * them (only really useful if you've installed them previously and don't
    * want them anymore).
    *
    * @see #createInstrument(JTree,boolean,boolean)
    ...
    *
    * @param b If true, will install the default editor. If false, does
    * nothing.
    *
   ...
   private void setUseDefaultCellEditor(boolean b)
   {
      if (b)
        m_tree.setCellEditor( new NodeInfoTreeCellEditor(
           m_tree,
           (javax.swing.tree.DefaultTreeCellRenderer)m_tree.getCellRenderer(),
           this));
   }
    ...
    * @see #setNodeFactory
    */
   public NodeFactory getNodeFactory() { return m_nodeFactory; }
   ...
   /**
    * Returns a reference to the currently active undo manager, if I know
    * about it. If I do not know about it, returns null.
    */
   ...
   /**
    * Registers a new undo manager with me. I take care of getting rid of
    * any old one (removing it as listener) and making the new one a listener.
    * I will also subscribe to its changeEvents if it is a "InfoUndoManager"
    * or subclass. If so, I will correctly update my undo and redo actions.
    * If I do not know about the current undo manager (i.e., this attribute
    * has been set to null) the actions will always be disabled. If I think
    * I know about it but do not (i.e., I have wrong information) the
    * state of these actions may be wrong.
    *
    * @see #getUndoAction
    * @see #getRedoAction
    */
   ...
   /**
    * Empties the undo buffer. Only works if I know about the current
    * undo manager.
    *
    * @see #setUndoManager
    */
    ...
    */
   private void getFocusForTree() { m_tree.requestFocus(); }

   /** Moves selected nodes one position towards the top */
   public void moveNodesUp() { moveNodes(true); };
   /** Moves selected nodes one position towards the bottom */
   public void moveNodesDown() { moveNodes(false); };
    ...
    *
    * @param nodes List of nodes to add to the selection. The nodes need
    * to be DefaultMutableTreeNode or descendants.
    */
   private void setSelection(Vector nodes)
   {
      ...
      while (en.hasMoreElements())
         m_tree.addSelectionPath(
            new TreePath(
               ((DefaultTreeModel)m_tree.getModel()).getPathToRoot(
                  (DefaultMutableTreeNode)en.nextElement())));
      m_tree.scrollPathToVisible(
         new TreePath(
            ((DefaultTreeModel)m_tree.getModel()).getPathToRoot(
               (DefaultMutableTreeNode)nodes.firstElement())));
   }

   /**
    * Returns an array of all selected nodes, sorted by their row number.
    *
    * @param ascending If true, nodes in low-numbered rows will be sorted first,
    * then nodes with increasing row numbers. If false, sort order will be
    * reverse of this.
    *
   ...
   TreePath[] getSortedSelection(boolean ascending)
   {
      ...
      if (retval==null) return new TreePath[0];
      final boolean asc = ascending;
      Arrays.sort(retval, new Comparator()
      {
         public int compare(Object a, Object b)
         {
            return (m_tree.getRowForPath((TreePath)a) -
               m_tree.getRowForPath((TreePath)b)) * (asc?1:-1);
         }
      });
      return retval;
   }
   ...
   /**
    * Moves a node in the tree. Makes sure that an undo object is created
    * and stored.
    *
    * @param node_to_move Reference to the node to move
    * @param dest_node New parent for the node to move
    * @param dest_index Index within dest_node that node_to_move should end
    * up in
    *
    * @throws VetoException The move was vetoed by one of the veto listeners
    */
   public void moveNode(
      DefaultMutableTreeNode node_to_move,
      DefaultMutableTreeNode dest_node,
      int dest_index)
      throws VetoException
   {
      m_undoSupport.postEdit(
         doMoveNode(node_to_move, dest_node, dest_index));
   }
    ...
    *
    * @param If true, moves nodes upwards (towards row 0), if false moves
    * downwards.
    */
      ...
      try
      {
         for (int i = 0; i<selrange.length; ++i)
         {
            DefaultMutableTreeNode node =
               (DefaultMutableTreeNode)selrange[i].getLastPathComponent();
            ...
            if (parent != null)
            {
               ...
               {
                  m_undoSupport.postEdit(
                     doMoveNode(node, parent, ndx+(up?-1:1)));
                  added = true;
               }
               else
               {
   ...
   /**
    * Moves all selected nodes one position to the "left" on the tree. This
    * has the effect of making the node the parent of what used to be the
    * siblings below it in the tree. If the node is at root level, nothing
    * happens to it. Expects nodes to be of type DefaultMutableTreeNode.
    */
      ...
      try
      {
         for (int i = 0; i < lastsel.length; ++i)
         {
            DefaultMutableTreeNode sel =
               (DefaultMutableTreeNode)lastsel[i].getLastPathComponent();
            nodeV.addElement(sel);
            DefaultMutableTreeNode oldparent =
               (DefaultMutableTreeNode)sel.getParent();
            if (oldparent==null) continue;
            DefaultMutableTreeNode newparent =
               (DefaultMutableTreeNode)oldparent.getParent();
            ...
            DefaultMutableTreeNode thisChild =
               (DefaultMutableTreeNode)oldparent.getChildAfter(sel);
            ...
            {
               nextChild =
                  (DefaultMutableTreeNode)oldparent.getChildAfter(thisChild);
               m_undoSupport.postEdit(
                  doMoveNode(
                     thisChild,
                     sel,
            ...
            added = true;
            m_tree.makeVisible(
               new TreePath(((DefaultTreeModel)m_tree.getModel())
                  .getPathToRoot(sel)));
            if (sel.getChildCount()>0) m_tree.makeVisible(new TreePath(
               ((DefaultTreeModel)m_tree.getModel())
                  .getPathToRoot(sel.getFirstChild())));
         }
      }
      catch (VetoException ex)
      {
   ...
   public void makeChild()
   {
      ...
      try
      {
         for (int i = 0; i < lastsel.length; ++i)
         {
            ...
            if (oldparent==null) continue;
            DefaultMutableTreeNode newparent =
               (DefaultMutableTreeNode)oldparent.getChildBefore(sel);
            if (newparent==null) continue;
            m_undoSupport.postEdit(
               doMoveNode(sel, newparent, newparent.getChildCount()));
            added = true;
            m_tree.makeVisible(
               new TreePath(((DefaultTreeModel)m_tree.getModel())
                  .getPathToRoot(sel)));
    ...
    *
    * @param parent Node to place new nodes within
    * @param nodes Array of nodes to add
    * @param parent_index Index in parent into which the first node in the
      ...
      int parent_index)
   {
      ...
      try
      {
         ...
         {
            m_undoSupport.postEdit(doAddNode(parent, nodes[i], parent_index));
            added = true;
         }
      }
      catch (VetoException ex)
    ...
    *
    * @param above If "true", will add to the top of the node's child array.
    * If false, will add to the bottom of it.
    *
    * @throws VetoException If the operation was vetoed by a
    * VetoableTreeStructureChangeListener. In this case, the operation did
    * not happen and any changes that were temporarily in effect got rolled
    * back.
    */
      ...
      throws VetoException
   {
      ...
      DefaultMutableTreeNode parentnode = (DefaultMutableTreeNode)
         m_tree.getLeadSelectionPath().getLastPathComponent();
      ...
      {
         initiate_edit = true;
         newnode = (DefaultMutableTreeNode)m_nodeFactory.createNode(parentnode);
      }
      else
      {
         if (newnodeinfo instanceof SubTreeNode)
         {
            ...
            newnode =
               (DefaultMutableTreeNode)m_nodeFactory.createNode( newnodeinfo );
      }
    ...
    * back.
    */
   private UndoableEdit addNode(boolean above)
      throws VetoException
   {
    ...
    * DefaultMutableTreeNodes. Uses the node factory to create new
    * nodes.
    *
    * @param above true to add above selection (or at top if no selection),
    * false to add below selection (or at bottom if no selection).
    * @param newnodeinfo Object to store as user object in new node, or
    * null for no contents.
    *
    * @return the UndoableEdit corresponding to the addition. This must
    * be registered with the undo system for correct operation.
    *
    * @see #setNodeFactory
    * @see #setUseDefaultNodeFactory
    *
      ...
      throws VetoException
   {
      ...
      if (numsel>0)
      {
         ...
         {
            if (selrows[i] > maxsel) maxsel = selrows[i];
            if (selrows[i] < minsel) minsel = selrows[i];
         }
         DefaultMutableTreeNode actsel;
         actsel =
            (DefaultMutableTreeNode)m_tree.getPathForRow(above?minsel:maxsel)
      ...
      if (((DefaultMutableTreeNode)m_tree.getModel().getRoot()).getChildCount()
         == 0)
      {
    ...
    *
    * @param node Node the ancestors of which to search for.
    * @param hash Hash to look for ancestors in. Will search the hash's
    * keys for ancestors, not the contents.
    *
    * @return true if one of the node's ancestors was found in the hash, false
    * if not.
    */
   static private boolean isNodeParentInHash(
      DefaultMutableTreeNode node,
      Hashtable hash)
   {
   ...
   /**
    * Deletes currently selected nodes with a "delete" semantic (as opposed
    * to a "backspace" semantic).
    *
    * @see #deleteSelection(boolean)
    */
   public void deleteSelectedNodes() { deleteSelection(true); }

   /**
    * Reduces a set of tree nodes to the minimum set that includes them
    * all. That is, if node n2 is a child of node n1 and both are included
    * in the input parameter, then node n2 will not be part of the return
    * value. n2 is implicitly included by the fact that n1 is
    * included with all its children. This is a utility method.
    *
    * @param original Set of tree nodes to reduce.
    *
    * @return Minimum set of tree nodes that includes all nodes in the
    * original when a node's children are assumed to be included implicitly
   ...
   public static TreePath[] reduceSelection(TreePath[] original)
   {
      ...
      for ( int i=0; i<original.length; ++i )
         hash.put(original[i].getLastPathComponent(),
            Boolean.TRUE);

      for (int i = 0; i < original.length; ++i)
      {
         DefaultMutableTreeNode node =
            (DefaultMutableTreeNode)original[i].getLastPathComponent();
    ...
    * "backspace" semantic. Nodes are deleted unconditionally. Expects
    * the selected nodes to be of type DefaultMutableTreeNode.
    *
    * @param remain If true, will delete with "delete" semantic. This means
    * that after deletion, the selected node will be at the same location as
    * the lead selection was at before deletion. If false, will delete with a
    * "backspace" semantic. That is, after the deletion, the selected node is
    * at the location immediately prior to what used to be the location of the
    * previous lead selection.
    */
   void deleteSelection(boolean remain)
   {
      ...
      if (lead>=m_tree.getRowCount()) lead = m_tree.getRowCount()-1;
      if (lead<0) lead = 0;
      m_tree.setSelectionRow(lead);
   }

   /**
    * Moves a node and creates an appropriate undo-action for it.
    * Expects the tree model to be of type DefaultTreeModel.
    *
    * @param node_to_move Reference to the node to be moved. This node should
    * already be in the tree. This can <em>NOT</em> be
    * the root node - this will cause NullPointerException to be thrown.
    * @param dest_node Reference to the new parent of the moving node. The
    * parent should be somewhere in the tree.
    * @param dest_index The index in the dest_node into which the
    * node_to_move should end up. Already existing nodes at and after this
    * index all get moved back one position.
    * @param undovec Vector into which to add the appropriate Undo-action for
    * this operation. If null, then adds the Undo-action to the tree's undo
    * vector. If not null, you <em>must</em> take care of adding this vector
    * to the tree's undo vector yourself. If you do not, undo/redo is going
    * to behave very strangely.
    *
    * @return the UndoableEdit corresponding to the addition. This must
    * be registered with the undo system for correct operation.
    */
   private UndoableEdit doMoveNode(
      DefaultMutableTreeNode node_to_move,
      DefaultMutableTreeNode dest_node,
      int dest_index)
      throws VetoException
   {
      fireNodeMove(new VetoableTreeStructureChangeEvent(
         this,
         node_to_move,
         node_to_move.getParent(),
         node_to_move.getParent().getIndex(node_to_move),
         dest_node,
         dest_index));
      ...
      UndoableEdit unedit = new UndoableInfoEdit(
         ...
         (DefaultMutableTreeNode)node_to_move.getParent(),
         node_to_move.getParent().getIndex(node_to_move),
         dest_node, dest_index,
         node_to_move,
         false);

      ((DefaultTreeModel)m_tree.getModel()).removeNodeFromParent(node_to_move);
    ...
    * Expects the tree model to be of type DefaultTreeModel.
    *
    * @param parent Reference to the parent of the new node. The
    * parent should be somewhere in the tree.
    * @param node Node to add to the tree. It should <em>not</em> be in the
    * tree already.
    * @param index The index in the parent into which the
    * node should end up. Already existing nodes at and after this
    * index all get moved back one position.
    * @param undovec Vector into which to add the appropriate Undo-action for
    * this operation. If null, then adds the Undo-action to the tree's undo
    * vector. If not null, you <em>must</em> take care of adding this vector
    ...
    * be registered with the undo system for correct operation.
    */
   private UndoableEdit doAddNode(
      DefaultMutableTreeNode parent,
      DefaultMutableTreeNode node,
      int index)
      throws VetoException
   {
      fireNodeCreation(
         new VetoableTreeStructureChangeEvent(
         this,
         node,
         null,
         0,
      ...
      UndoableEdit unedit = new UndoableInfoEdit((DefaultTreeModel)m_tree.getModel(),
      												UndoableInfoEdit.TYPE_ADD,
													null, 0,
													parent, index,
													node, true);
      ((DefaultTreeModel)m_tree.getModel()).insertNodeInto(node, parent, index);

      return unedit;
   }

   /**
    * Deletes a node from the tree and creates an appropriate undo-action
    * for it.
    *
    * @param node Node to delete from the tree. This can <em>NOT</em> be
    * the root node - this will cause NullPointerException to be thrown.
    * @param undovec Vector into which to add the appropriate Undo-action for
    * this operation. If null, then adds the Undo-action to the tree's undo
    * vector. If not null, you <em>must</em> take care of adding this vector
   ...
   private UndoableEdit doDeleteNode(DefaultMutableTreeNode node)
      throws VetoException
   {
      fireNodeDeletion(new VetoableTreeStructureChangeEvent(
         this,
         node,
         node.getParent(),
   ...
   /**
    * Will ensure that the Action objects we have created are in sync
    * with the selection state of the tree. This method should be called
    * whenever there is reason to believe that the selection in the tree
    * has changed.
    */
   ...
   /**
    * Utility method that hands out unique IDs to use for identifying drag
    * and drop operations. If you do not intend to do some low-level DND
    * coding, you do not need this method.
    *
    * @return A new DND ID that is guaranteed to be unique for this instance
    * of Instrumenter. Note that the unique id is implemented as a long, so
    * the potential for a wrap-around and following non-uniqueness of the ids
    * is present.
    */
    ...
    * If you do not intend to do some low-level DND
    * coding, you do not need this method.
    *
    * @param op_id Operation ID of DND operation to finish
    * @param source Set to true if the source is done handling the DND
    * operation, false if it is not done.
    */
   public void finishDndOperation(long op_id, boolean source)
   {
      ...
      {
         DndOperations op = (DndOperations)m_hashDndOperations.get(op_id_ob);
         if (op!=null)
         {
            if (source)
            ...
            if (op.m_bSourceDone)
            {
               ...
               for (int i=0; i<op.m_ops.size(); ++i)
                  m_undoSupport.postEdit((UndoableEdit)op.m_ops.elementAt(i));
               m_undoSupport.endUpdate();
    ...
    * time. It is, however, vital to make drag and drop behave correctly.
    * It will be investigated and documented as time permits.
    */
   public void doDndCompoundAdd(
      long op_id,
      DefaultMutableTreeNode parent,
      DefaultMutableTreeNode node,
      int index)
      throws VetoException
   {
      //System.out.println("doDndCompoundAdd(): Adding " + node + " to parent " + parent);
      UndoableEdit unedit = doAddNode(parent,node,index);
      Long op_id_ob = new Long(op_id);
      synchronized(m_hashDndOperations)
      {
         DndOperations dndop = (DndOperations)m_hashDndOperations.get(op_id_ob);
         if (dndop==null)
         {
    ...
    * It will be investigated and documented as time permits.
    */
   public void doDndCompoundDelete(
      long op_id,
      DefaultMutableTreeNode node)
      throws VetoException
   {
    ...
    * Does <em>not</em> put them into the system clipboard. Expects nodes
    * to be of type DefaultMutableTreeNode.
    *
    ...
    * Does <em>not</em> put them into the system clipboard. Expects
    * nodes to be of type DefaultMutableTreeNode.
    *
    * @see #m_arrCopiedNodes
    */
   public void copyNode()
   {
      ...
      for (int i=0; i<paths.length; ++i)
      {
         ...
         copiedNodes.addElement( new SubTreeNode(
            (DefaultMutableTreeNode)paths[i].getLastPathComponent(), true));
      }
   ...
   /**
    * Makes copies of the nodes in the local clipboard (<em>not</em> the
    * system clipboard) in the currently focused location. Expects nodes to
    * be of type DefaultMutableTreeNode.
    *
    * @see #m_arrCopiedNodes
    */
   ...
   /**
    * Copies nodes into the currently focused location. Expects nodes to
    * be of type DefaultMutableTreeNode.
    *
    ...
    * infobank.dtd and creates a new tree based upon the information.
    * The existing tree is deleted unconditionally.
    * </p>
    * <p>
    ...
    */
/*
   public String createFromXml(InputStream is)
   {
      try
      {
         ...
         TreeNode new_root_node = makeNodeFromXMLRecursive(root, true);
         DefaultTreeModel model = (DefaultTreeModel)m_tree.getModel();
         model.setRoot(new_root_node);
         purgeUndoBuffer();
      }
      catch (NoClassDefFoundError ex)
      {
         return "Could not find JDOM library.";
      }
      catch (org.jdom.JDOMException ex2)
      {
...
/*
   protected MutableTreeNode makeNodeFromXMLRecursive(
      org.jdom.Element xml_elem,
      boolean is_root)
   {
      ...
      java.util.List childlist = xml_elem.getChildren();
      int num_children = childlist.size();
      for (int i=0; i<num_children; ++i)
      {
         org.jdom.Element elem = (org.jdom.Element)childlist.get(i);
         if (is_root)
            title = null;
         else
         {
            if (elem.getName().equals("name"))
               title = elem.getText();
            else if (elem.getName().equals("contents"))
               contents = elem.getText();
         }
...
*/
    ...
    * </p>
    *
    * @param os Outputstrem to send XML to
    */
   /*
      ...
      throws IOException
   {
      ...
      org.jdom.DocType type = new org.jdom.DocType("tree", "infobank.dtd");
   ...
   protected org.jdom.Element makeXMLNodeRecursive(Object node, boolean is_root)
   {
      org.jdom.Element xml_node = new org.jdom.Element(is_root?"tree":"node");
      if (node==null)
      {
         return xml_node;
      }
      ...
      if (!is_root)
      {
         NodeInfo info = (NodeInfo)tnode.getUserObject();

         if (((NodeInfo)info).getTitle().length()>0)
         {
            org.jdom.Element name = new org.jdom.Element("name");
            name.addContent(info.getTitle());
            xml_node.addContent(name);
         }

         if (info.getContents().length()>0)
         {
            org.jdom.Element contents = new org.jdom.Element("contents");
            contents.addContent(info.getContents());
...
*/
   ...
   /**
    * An array of all actions that are enabled only when 1+ nodes
    * are selected
    */
    ...
    */
   public long getMyInstrumentId() { return m_nMyInstrumentId ; }
   ...
   /**
    * Adds a listener to changes in the tree structure. The listener can
    * veto any changes.
    *
   ...
   public synchronized void addVetoableTreeStructureChangeListener(
      VetoableTreeStructureChangeListener l)
   {
      Vector v = vetoableTreeStructureChangeListeners == null ?
         new Vector(2) :
    ...
    * @param Event to send
    *
    * @throws VetoException if one of the listeners didn't like the change. In
