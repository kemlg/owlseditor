...
public class TreeDropTargetListener implements DropTargetListener {
	...
	private Timer m_timerScroll = new Timer(100, new ActionListener() {
		public void actionPerformed(ActionEvent ev) {
			maybeScrollList();
		}
	});
	private Timer m_timerExpand = new Timer(1500, new ActionListener() {
		public void actionPerformed(ActionEvent ev) {
	...
	protected boolean checkFlavourSupported(DataFlavor[] flavours) {
		/*
		 * for (int i=0; i<flavours.length; ++i) if (
		 ...
		 * +DataFlavor.getTextPlainUnicodeFlavor().getMimeType() +" - Primary="
		 * +DataFlavor.getTextPlainUnicodeFlavor().getPrimaryType()
		 * +" - Secondary=" +DataFlavor.getTextPlainUnicodeFlavor().getSubType()
	...
	public synchronized void drop(DropTargetDropEvent dropDropEvent) {
		try {
			...
			TreePath targetpath = m_tree
					.getClosestPathForLocation(loc.x, loc.y);
			if (targetpath != null) {
				parentnode = (DefaultMutableTreeNode) targetpath
				...
				parentnode = (DefaultMutableTreeNode) m_tree.getModel()
						.getRoot();
				parentindex = 0;
			}
			...
			if (parentnode != null) {
				...
				if (true) {
					...
					DefaultTransferInfo info = (DefaultTransferInfo) dropDropEvent
							.getTransferable().
							// getTransferData(DefaultTransferable.DEFAULT_NODE_INFO_FLAVOUR);
							getTransferData(
									OWLSDataFlavorProvider.getInstance()
											.getOWLSDataFlavor());

					boolean is_same_tree = dropDropEvent.isLocalTransfer()
							&& (m_instrument.getMyInstrumentId() == info
									.getDndId().getInstrumentId());
					...
					if (is_same_tree) {
						// Source and destination tree is the same - move the
						// node

						// First check if I am trying to move a node into itself
						// in
						// some fashion. This must not be allowed as the results
						// of doing so are weird.
						// We assume that the nodes involved in the DnD
						// operation
						// are the ones that are still selected in the tree. If
						// this
						// does not hold, this algorithm will fail.
						DefaultMutableTreeNode iter = parentnode;
						do {
							if (m_tree.isPathSelected(new TreePath(iter
									.getPath()))) {
								return;
							}
						} while ((iter = (DefaultMutableTreeNode) iter
								.getParent()) != null);

						// Then do the move.
						DefaultMutableTreeNode[] newnodes = new DefaultMutableTreeNode[nodes.length];
						for (int i = nodes.length - 1; i >= 0; --i) {
							// System.out.println("Node: " + nodes[i] + " of " +
							// nodes[i].getClass() +
						...
						System.out
								.println("TreeDropTargetListener.drop(): Moving nodes to new array"); // Source
																										// and
																										// destination
																										// are
																										// different
						...
						m_instrument
								.addNodes(parentnode, newnodes, parentindex);
					}
					dropDropEvent.getDropTargetContext().dropComplete(
							!drop_failed);
					if (drop_failed && added_undo) {
						m_instrument.getUndoManager().undo();
					}
				} else if (dropDropEvent.getTransferable()
						.isDataFlavorSupported(DataFlavor.stringFlavor)) {
					dropDropEvent.acceptDrop(DnDConstants.ACTION_COPY);

					String info = (String) dropDropEvent.getTransferable()
					...
					DefaultMutableTreeNode[] arr_newnodes = new DefaultMutableTreeNode[newnodes
							.size()];
					newnodes.copyInto(arr_newnodes);
					m_instrument
							.addNodes(parentnode, arr_newnodes, parentindex);
					dropDropEvent.getDropTargetContext().dropComplete(true);
				} else if (dropDropEvent.getTransferable()
						.isDataFlavorSupported(DataFlavor.javaFileListFlavor)) {
					dropDropEvent.acceptDrop(DnDConstants.ACTION_COPY);

					List info = (List) dropDropEvent.getTransferable()
							.getTransferData(DataFlavor.javaFileListFlavor);
					DefaultMutableTreeNode[] newnodes = new DefaultMutableTreeNode[info
							.size()];
					...
					m_instrument.addNodes(parentnode, newnodes, parentindex);
					dropDropEvent.getDropTargetContext().dropComplete(true);
				} else if (checkForMimeType(dropDropEvent.getTransferable()
						.getTransferDataFlavors(), "text/plain") != null) {
					dropDropEvent.acceptDrop(DnDConstants.ACTION_COPY);
					DataFlavor flavour = checkForMimeType(dropDropEvent
							.getTransferable().getTransferDataFlavors(),
							"text/plain");

					Reader info = flavour.getReaderForText(dropDropEvent
							.getTransferable());
					BufferedReader br = new BufferedReader(info);
					Vector newnodes = new Vector();
					while (br.ready()) {
						Object ob = m_instrument.getNodeFactory().createNode(
								br.readLine(), "");
						if (br.ready())
	...
	private void checkDragOver(DropTargetDragEvent ev) {
		...
		if (ev == null) {
			...
			if ((locIsOnUppermostRow(loc) && !checkScrollBarAtTop())
				...
				if (path != null
						&& !m_tree.getModel().isLeaf(
								path.getLastPathComponent())) {
					boolean expand = !m_tree.isExpanded(path);
					if (m_bAllowExpand
							&& (!m_timerExpand.isRunning()
									|| !path.equals(m_pathToExpand) || m_bExpandActionIsExpand != expand)) {
						m_timerExpand.stop();
			...
			if (m_dragOverNode != null) {
				old_row = m_tree.getRowForPath(new TreePath(m_dragOverNode
						.getPath()));
				if (m_dragOverNode.getUserObject() instanceof DragOverData) {
					old_effect = ((DragOverData) m_dragOverNode.getUserObject())
							.getCurrentEffect();
				}
				if (old_row != row)
			...
			if (row != -1) {
				new_node = (DefaultMutableTreeNode) m_tree.getPathForRow(row)
						.getLastPathComponent();
	...
	private void activateNewDragOverEffect(DefaultMutableTreeNode node,
			int effect) {
		if (node.getUserObject() instanceof DragOverData) {
			DragOverData dod = (DragOverData) node.getUserObject();
			dod.setCurrentEffect(effect);
			m_tree.getModel().valueForPathChanged(new TreePath(node.getPath()),
					dod);
		}
	}
	...
	private boolean locIsOnUppermostRow(Point loc) {
		...
		int top_row = m_tree.getClosestRowForLocation(0,
				m_tree.getVisibleRect().y + height);
		if (top_row != -1) {
	...
	private boolean locIsOnLowermostRow(Point loc) {
		...
		if (height <= 0)
			height = 10; // Best guess
		int bot_row = m_tree.getClosestRowForLocation(0,
				m_tree.getVisibleRect().y + m_tree.getVisibleRect().height
						- height);
		if (bot_row != -1) {
	...
	protected DefaultMutableTreeNode makeFileNode(File file) {
		DefaultMutableTreeNode node = (DefaultMutableTreeNode) m_instrument
				.getNodeFactory().createNode(file.getName(), "");
		if (file.isDirectory()) {
