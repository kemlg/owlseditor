...
/**
 * This is the default transferable that is used for transfering node data
...
public class DefaultTransferable implements Transferable {
	...
	final public static DataFlavor DEFAULT_NODE_INFO_FLAVOUR = new DataFlavor(
			DefaultNodeInfo.class, "Default Node Info");
	...
	static {
		 ...
		 * v.addElement(DataFlavor.javaFileListFlavor); addMimeType(v,
		 * "text/plain; charset=UTF-16BE"); addMimeType(v,
		 * "text/plain; charset=UTF-16LE"); addMimeType(v,
		 * "text/plain; charset=UTF-16"); addMimeType(v,
		 * "text/plain; charset=ISO-8859-1"); addMimeType(v,
		 * "text/plain; charset=UTF-8"); addMimeType(v,
		 * "text/plain; charset=US-ASCII"); addMimeType(v,
	...
	/**
	 * Utility method to add data flavours to a vector. Will not add them if
	 * they turn out to be invalid.
	 * 
	 * @param v
	 *            Vector to add to
	 * @param mimetype
	 *            name of mime type to add
	 */
	...
	public DefaultTransferable(DefaultMutableTreeNode node, DndId dndId) {
		m_data = new DefaultTransferInfo(new SubTreeNode[] { new SubTreeNode(
				node, true) }, dndId);
	}
	...
	public DefaultTransferable(DefaultMutableTreeNode[] nodes, DndId dndId) {
		...
		m_data = new DefaultTransferInfo(stn, dndId);
	}
	...
	public Object getTransferData(DataFlavor flavor)
		...
		Object retval;
		if (flavor.equals(OWLSDataFlavorProvider.getInstance()
				.getOWLSDataFlavor())) {
