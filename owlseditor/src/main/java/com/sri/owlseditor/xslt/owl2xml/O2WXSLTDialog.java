...
public class O2WXSLTDialog extends XSLTDialog {
	...
	private JButton createTag, createData, createVariable, createAttribute,
			deleteNode;

	public O2WXSLTDialog(OWLModel okb, OWLIndividual aP, String wsdl,
			String wsdlM, OWLIndividual owlInst) throws XSLTException {
		super(okb, wsdl);
		atomicProcess = aP;
		...
		OWLDatatypeProperty wsdlMessagePartProp = okb
				.getOWLDatatypeProperty("grounding:wsdlMessagePart");
		RDFSLiteral wsdlpart = (RDFSLiteral) owlInst
				.getPropertyValue(wsdlMessagePartProp);
		if (wsdlpart == null)
		...
		if (wsdlInputMessage == null)
			throw new XSLTException("WSDL Input Message " + wsdlM
					+ " not found.");
		// WSDL4J sucks. It expects just the local part of the message part uri.
		wsdlInputMessagePart = wsdlInputMessage.getPart(wsdlparturi
				.substring(wsdlparturi.indexOf('#') + 1));
		if (wsdlInputMessagePart == null)
			throw new XSLTException("WSDL Input Message Part " + wsdlparturi
					+ " not found.");
	...
	protected void setupGUI() {
		...
		JLabel targetWsdlTypeLabel = ComponentFactory
				.createLabel("Target WSDL type");
		JTextArea targetWsdlTypeArea = ComponentFactory.createTextArea();
		targetWsdlTypeArea.setEditable(false);
		JScrollPane targetWsdlTypePane = ComponentFactory
				.createScrollPane(targetWsdlTypeArea);
		...
		JLabel sourceOWLTypesLabel = ComponentFactory
				.createLabel("Source OWL Types");
		JTextArea sourceOWLTypesArea = ComponentFactory.createTextArea();
		sourceOWLTypesArea.setEditable(false);
		JScrollPane sourceOWLTypesPane = ComponentFactory
				.createScrollPane(sourceOWLTypesArea);
	...
	protected void prepareTree() {
		...
		if (wsdlInputMessagePart != null) { // "part" was found
			...
			if (type.getNamespaceURI().equalsIgnoreCase(
	...
	protected void createTranslationPanel() {
		if (transPanel == null)
			transPanel = new O2WParameterPanel(this);
	}
	...
	protected void addNewNode(XSLTNode n) {
		...
		if (n instanceof VariableNode) {
			((O2WParameterPanel) transPanel).getDataPanel().addParameter(
					"$" + n.getNodeName());
			((O2WParameterPanel) transPanel).getAttributePanel().addParameter(
					"$" + n.getNodeName());
			((O2WParameterPanel) transPanel).getVariablePanel().addParameter(
					"$" + n.getNodeName());
