...
public class RepeatUntilNode extends ConditionalNode {
	...
	public GraphNodeInfo graph(HashSet nameSet, PrintWriter pw,
		...
		if (e.hasMoreElements()) {
			...
			processPair = processNode.graph(nameSet, pw, newClusterNumber++,
					selectedNode);
			...
			pw.println(condNodeName.getUniqueName() + "->"
					+ processPair.firstNode + "[label=\"false\"];");

			pw.println("}");
			return new GraphNodeInfo(processPair.firstNode,
					condNodeName.getUniqueName(), processPair.inEdgeAttr,
					"label=\"true\"", newClusterNumber);
		} else {
			pw.println("}");
			return new GraphNodeInfo(condNodeName.getUniqueName(),
					condNodeName.getUniqueName(), "", "label=\"true\"",
					newClusterNumber);
		}
	}
	...
	private OWLIndividual getUntilProcess() {
		OWLIndividual inst = (OWLIndividual) OWLUtils.getNamedSlotValue(
				getInstance(), "process:untilProcess", getOWLModel());
		return inst;
	}

	public void updateKBAfterMove(OWLSTreeNode node) {
		OWLUtils.removeNamedSlotValue(getInstance(), "process:untilProcess",
				getOWLModel());
	}
	...
	public void updateKBAfterInsert(OWLSTreeNode newnode) {
		OWLUtils.setNamedSlotValue(getInstance(), "process:untilProcess",
