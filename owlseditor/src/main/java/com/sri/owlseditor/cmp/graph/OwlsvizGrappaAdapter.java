...
public class OwlsvizGrappaAdapter implements GrappaConstants, GrappaListener {
	...
	private void createPopupMenus() {
		...
		findInputProviderItem
				.addActionListener(new FindInputProviderListener());
		rightClickMenu.add(findInputProviderItem);

		JMenuItem findOutputConsumerItem = new JMenuItem("Find output consumer");
		findOutputConsumerItem
				.addActionListener(new FindOutputConsumerListener());
		rightClickMenu.add(findOutputConsumerItem);

		JMenuItem findMatchingProcessItem = new JMenuItem(
				"Find matching process");
		findMatchingProcessItem
				.addActionListener(new FindMatchingProcessListener());
		 ...
		 * 
		 * JMenuItem cfindInputProviderItem = new
		 * JMenuItem("Find input provider");
		 * cfindInputProviderItem.addActionListener(new
		 * FindInputProviderListener());
		 * compositeRightClickMenu.add(cfindInputProviderItem);
		 * 
		 * JMenuItem cfindMatchingProcessItem = new
		 * JMenuItem("Find matching process");
		 * cfindMatchingProcessItem.addActionListener(new
		 * FindMatchingProcessListener());
		 * compositeRightClickMenu.add(cfindMatchingProcessItem);
		 * 
		 * JMenuItem showCompositeProcessItem = new
		 * JMenuItem("Show this composite process");
		 * showCompositeProcessItem.addActionListener(new
		 * ShowCompositeProcessListener());
	...
	public void grappaPressed(Subgraph subg, Element elem, GrappaPoint pt,
		...
		if ((modifiers & (InputEvent.BUTTON2_MASK | InputEvent.BUTTON3_MASK)) != 0
			...
			/*
			 * if (isCompositePerform(elem)) compositeRightClickMenu.show(panel,
			 * (int)mpt.getX(), (int)mpt.getY()); else
