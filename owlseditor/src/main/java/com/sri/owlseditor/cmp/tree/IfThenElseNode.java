...
public class IfThenElseNode extends ConditionalNode {
	...
	public GraphNodeInfo graph(HashSet nameSet, PrintWriter pw,
		...
		if (e.hasMoreElements()) {
			...
			GraphNodeInfo ifPair = ifNode.graph(nameSet, pw,
			...
			pw.println(condNodeName.getUniqueName() + "->"
					+ endNodeName.getUniqueName());
		}
		if (e.hasMoreElements()) {
			OWLSTreeNode elseNode = (OWLSTreeNode) e.nextElement();
			GraphNodeInfo elsePair = elseNode.graph(nameSet, pw,
					newClusterNumber++, selectedNode);
			...
			pw.println(condNodeName.getUniqueName() + "->"
					+ endNodeName.getUniqueName() + " [label=\"false\"];");
		}
	...
	public OWLIndividual getThenProcess() {
		return (OWLIndividual) OWLUtils.getNamedSlotValue(getInstance(),
				"process:then", getOWLModel());
	}

	public OWLIndividual getElseProcess() {
		return (OWLIndividual) OWLUtils.getNamedSlotValue(getInstance(),
				"process:else", getOWLModel());
	}

	public void updateKBAfterDelete(OWLSTreeNode node) {
		int index = getIndex(node);
		/*
		 * For now, we do the same thing for both cases, but there should be
		 * some special handling of the various cases here.
		 */
		if (index == 0) {
			// then case
	...
	public void updateKBAfterMove(OWLSTreeNode node) {
		...
		if (index == 0) {
			// then case
			OWLUtils.removeNamedSlotValue(getInstance(), "process:then",
					getOWLModel());
		} else if (index == 1) {
			// else case
			OWLUtils.removeNamedSlotValue(getInstance(), "process:else",
					getOWLModel());
		}
	}
	...
	public void updateKBAfterInsert(OWLSTreeNode newnode) {
		...
		if (newindex == 0) {
			// then-case
			OWLUtils.setNamedSlotValue(getInstance(), "process:then",
					newnode.getInstance(), getOWLModel());
		} else if (newindex == 1) {
			// else-case
			OWLUtils.setNamedSlotValue(getInstance(), "process:else",
					newnode.getInstance(), getOWLModel());
		}
	}
