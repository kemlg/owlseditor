...
public class OWLSList implements Iterator {
	...
	// Note that the following two are deprecated, so we'll proably never use
	// them.
	...
	private OWLIndividual next; // should always point to the next element in
								// the list
	...
	/*
	 * public OWLSList(OWLIndividual inst, String type, OWLModel okb) { next =
	 * inst; this.okb = okb; this.type = type;
	 * 
	 ...
	 * OWLNamedClass listCls = okb.getOWLNamedClass(type); Instance newcell =
	 * listCls.createDirectInstance(null); OWLUtils.setNamedSlotValue(newcell,
	 * "list:rest", okb.getInstance("list:nil"), okb); return newcell; }
	 */

	/*
	 * Creates a list cell for the correct type of list/bag, and puts inst as
	 * the value of the list:first property
	 */
	private OWLIndividual createCell(OWLIndividual inst) {
		...
		// System.out.println("Creating new list cell with name " +
		// newcell.getName() +
	...
	private void removeCell(OWLIndividual cell, boolean deleteInst) {
		...
		// System.out.println("removeCell() removing list cell " +
		// cell.getName());
		if (deleteInst) {
	...
	private void printCell(OWLIndividual cell) {
		if (cell == getNil())
			...
			System.out.println("  element (list:first value): "
					+ first.getName());
	 ...
	 * the list:first property on the list cell, and updates the list:rest
	 * pointers of the list to give this cell its correct position in the list.
	 * 
	 * If the index to insert at is 0, the return value will be the new list
	 * cell for inst. This is needed so that the caller can update the
	 * process:components property (or similar property) of the containing
	 * control construct. If index is not 0, the return value should be null.
	 */
	public void insertAtIndex(OWLIndividual inst, int index) {
		// System.out.println("inserting " + inst.getName() + " at index " +
		// index);
		...
		if (i < index) {
			 ...
			 */
			System.out
					.println("WARNING: insertAtIndex() could not reach the desired node index: "
							+ index);
			// return null;
			return;
		} else {
			...
			if (i == 0) {
				...
				// We return the new cell so caller can update containing
				// control construct
	...
	/**
	 * Remove the list cell at the given index, and the control constructs and
	 * list cells below it, but not processes pointed to by Performs, etc.
	 * 
	 * If the index to remove at is 0, the return value will be the new first
	 * list cell This is needed so that the caller can update the
	 * process:components property (or similar property) of the containing
	 * control construct. If index is not 0, the return value should be null.
	 * 
	 * @param deleteInst
	 *            If true, the control construct contained in the node will also
	 *            be deleted. If false, only the list cell will be removed. The
	 *            latter case is used when a node is moved using drag and drop.
	 * 
	 */
	public void removeAtIndex(int index, boolean deleteInst) {
		...
		if (index == 0) {
			/*
			 * this requires some special handling (setting the components
			 * property of the containing control construct to point to the cell
			 * on index 1)
			 */
			...
			// return nextvalue; // caller must update containing control
			// construct
		} else {
			int i = 0;
			OWLIndividual dummy = null; // we don't use this
			...
			if (i < index) {
				 ...
				 */
				System.out
						.println("WARNING: removeAtIndex() could not reach the desired node index: "
								+ index);
			} else {
				...
				if (followingcell == null) {
					System.out
							.println("ERROR! following cell is null! This cannot happen");
					// return null;
					return;
				}

				// set list:rest pointer on previous node to following node
				// System.out.println("Setting list:rest on previous cell: " +
				// cell.getName() +
	...
	public OWLIndividual first() {
		OWLIndividual first = (OWLIndividual) controlConstruct
				.getPropertyValue(componentsProperty);
		next = first;
		return first;
	}
	...
	public Object next() {
		...
		// System.out.println("OWLSList.next() returning " +
		// ((Instance)retval).getName());
		// System.out.println("OWLSList.next() setting next to " +
		// next.getName());
		return retval;
	}
