 ...
 * edu.stanford.smi.protege.ui.DirectInstancesList We need to change some
 * internals, so pure subclassing is not enough. Changes to this file are kept
 * to a minimum, and the subclass OWLSInstanceList is used when possible. That
 * way, we can diff this file with the original DirectInstancesList.java and
 * integrate future changes to it easily.
 * 
 * 1) Commented out stuff that didn't compile (see comments below) 2) Changed
 * class labels to only list main class, not all subclasses (updateLabel()
 * method). 3) Added implements CommonOWLSInstancesList 4) Removed the toppanel
 * 5) Added a getClses() method, as it is needed by some subclasses. 6) Support
 * for checkbox panel for the IOPR manager.
 * 
 * --- The panel that holds the list of direct instances of one or more classes.
 * If only one class is chosen then you can also create new instances of this
 * class.
 * 
...
public class OWLSDirectInstancesList extends SelectableContainer implements
	 ...
	 * @param project
	 * @param checkboxes
	 *            true if checkboxes are desired, false otherwise.
	 */
	public OWLSDirectInstancesList(Project project, boolean checkboxes) {
		...
		if (checkboxes) {
			...
			checkboxPanel = new ListAndCheckboxPanel(_list,
					(OWLModel) _project.getKnowledgeBase());
			_labeledComponent = new LabeledComponent(null,
					ComponentFactory.createScrollPane(checkboxPanel));
		} else {
			// setLayout(new BorderLayout(10, 10));
			_labeledComponent = new LabeledComponent(null,
					ComponentFactory.createScrollPane(_list));
		}

		addButtons(viewAction, _labeledComponent);
		_labeledComponent.setFooterComponent(new ListFinder(_list,
				ResourceKey.INSTANCE_SEARCH_FOR));
	...
	private void updateLabel() {
		/*
		 * String text; Cls cls = getSoleAllowedCls(); BrowserSlotPattern
		 * pattern = (cls == null) ? null : cls.getBrowserSlotPattern(); if
		 * (pattern == null) { text = null; } else { // text = "Instances by ";
		 * if (pattern.isSimple()) { text =
		 * pattern.getFirstSlot().getBrowserText(); } else { text =
		 * "multiple slots"; } } _labeledComponent.setHeaderLabel(text);
		 */
	...
	private HeaderComponent createHeader() {
		...
		String instanceBrowserLabel = LocalizedText
				.getText(ResourceKey.INSTANCE_BROWSER_TITLE);
		String forClassLabel = LocalizedText
				.getText(ResourceKey.CLASS_EDITOR_FOR_CLASS_LABEL);
		_header = new HeaderComponent(instanceBrowserLabel, forClassLabel,
				label);
		...
		_createAction = new CreateAction(ResourceKey.INSTANCE_CREATE) {
			public void onCreate() {
				...
				if (_clses.size() > 1) {
					...
					rootClses.add(cls);
					cls = DisplayUtilities.pickCls(_labeledComponent, kb,
							rootClses, "Choose a class");
				} else {
					cls = (Cls) CollectionUtilities.getFirstItem(_clses);
				}
	...
	/*
	 * protected Action createConfigureAction() { return new ConfigureAction() {
	...
	/*
	 * We don't need this method, and it relies on
	 * createSetDisplaySlotMultipleAction(), which has problems. protected JMenu
	 * createSetDisplaySlotAction() { JMenu menu =
	 * ComponentFactory.createMenu("Set Display Slot"); boolean enabled = false;
	 * Cls cls = getSoleAllowedCls(); if (cls != null) { BrowserSlotPattern
	 * pattern = cls.getBrowserSlotPattern(); Slot browserSlot = (pattern !=
	 * null && pattern.isSimple()) ? pattern.getFirstSlot() : null; Iterator i =
	 * cls.getVisibleTemplateSlots().iterator(); while (i.hasNext()) { Slot slot
	 * = (Slot) i.next(); JRadioButtonMenuItem item = new
	 * JRadioButtonMenuItem(createSetDisplaySlotAction(slot)); if
	 * (slot.equals(browserSlot)) { item.setSelected(true); } menu.add(item);
	 * enabled = true; } JRadioButtonMenuItem item = new
	 * JRadioButtonMenuItem(createSetDisplaySlotMultipleAction()); if
	 * (browserSlot == null) { item.setSelected(true); } menu.add(item); }
	 * menu.setEnabled(enabled); return menu; }
	 */
	...
	/*
	 * MultiSlotPanel doesn't resolve, and we don't need this anyway protected
	 * Action createSetDisplaySlotMultipleAction() { return new
	 * AbstractAction("Multiple Slots...") { public void
	 * actionPerformed(ActionEvent event) { Cls cls = getSoleAllowedCls();
	 * BrowserSlotPattern currentPattern =
	 * getSoleAllowedCls().getBrowserSlotPattern(); MultiSlotPanel panel = new
	 * MultiSlotPanel(currentPattern, cls); int rval =
	 * ModalDialog.showDialog(ModifiedDirectInstancesList.this, panel,
	 * "Multislot Display Pattern", ModalDialog.MODE_OK_CANCEL); if (rval ==
	 * ModalDialog.OPTION_OK) { BrowserSlotPattern pattern =
	 * panel.getBrowserTextPattern(); if (pattern != null) {
	 * cls.setDirectBrowserSlotPattern(pattern); } } updateLabel(); repaint(); }
	 * }; }
	 */
		...
		_deleteAction = new DeleteInstancesAction(this) {
			...
			protected void onAboutToDelete(Object o) {
				...
				if (o instanceof OWLIndividual) {
					...
					OWLNamedClass expressionCls = model
							.getOWLNamedClass("expr:Expression");
					if (inst.hasRDFType(expressionCls, true)) {
						OWLObjectProperty expressionObject = model
								.getOWLObjectProperty("expr:expressionObject");
						RDFResource expr = (RDFResource) inst
								.getPropertyValue(expressionObject);
						if (expr != null && expr instanceof SWRLAtomList)
							SWRLWidget.deleteAtomList((SWRLAtomList) expr,
									model);
					}
				}
			}
	...
	private void updateButtons() {
		...
		boolean allowed = instance != null
				&& instance instanceof SimpleInstance;
