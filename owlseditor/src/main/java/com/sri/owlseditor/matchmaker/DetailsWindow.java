...
public class DetailsWindow {
	...
	private final static Color EQUIVALENT_COLOR = new Color(16, 68, 13); // dark
																			// green
	private final static Color SUBSUMES_COLOR = new Color(64, 145, 60); // lighter
																		// green
	private final static Color SUBSUMED_COLOR = new Color(230, 119, 0); // dark
																		// orange
	private final static Color FAIL_COLOR = new Color(183, 30, 0); // dark red
	private final static Color EXTRA_COLOR = new Color(119, 40, 115); // dark
																		// purple
	...
	private class TableRow {
		...
		public TableRow(OWLIndividual oldParameter, RDFResource oldType,
	...
	private class DetailsTableModel extends AbstractTableModel {
		...
		private String[] columnNames = { "Original process parameter", "Type",
				"Matching process parameter", "Type", "Type of match" };
		...
		/*
		 * public void insertRow(OWLIndividual oldParameter, String
		 * newParameter, String newType, int matchType){ if (parameterType ==
		 * null){ model = oldParameter.getOWLModel(); parameterType =
		 * model.getOWLDatatypeProperty("process:parameterType"); }
		 * 
		 * RDFResource oldType = null; if (oldParameter != null){
		 * DefaultRDFSLiteral oldTypeLiteral = (DefaultRDFSLiteral)oldParameter.
		 * getPropertyValue(parameterType); String oldTypeName =
		 * model.getResourceNameForURI(oldTypeLiteral.getString()); oldType =
		 * model.getRDFResource(oldTypeName); }
		 * 
		 * TableRow row = new TableRow(oldParameter, oldType, newParameter,
		 * newType, matchType); parameters.add(row); fireTableDataChanged(); }
		 */
	}
	...
	private DetailsWindow(JFrame parent) {
		...
		dialogContents
				.setLayout(new BoxLayout(dialogContents, BoxLayout.Y_AXIS));
