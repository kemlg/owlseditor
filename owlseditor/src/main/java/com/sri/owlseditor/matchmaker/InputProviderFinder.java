...
public class InputProviderFinder extends MatchFinder {
		...
		while (it.hasNext()) {
			...
			if (pair != null) {
				...
				if (outputstring != null) {
					OWLIndividual output = (OWLIndividual) model
							.getRDFResource(outputstring);
					if (output != null) {
	...
	/**
	 * Adds the match in a Sequence before the old perform, and updates the
	...
	private void addBefore(OWLIndividual oldPerform, MatchResult mResult) {
		...
		// System.out.println("addBefore: parent construct is " +
		...
		if (newPerform == null) {
			...
			if (parentConstruct == null) {
				...
				OWLIndividual sequenceConstruct = sequenceCls
						.createOWLIndividual(null);
				OWLNamedClass cclistCls = model
						.getOWLNamedClass(OWLSList.CC_LIST);
