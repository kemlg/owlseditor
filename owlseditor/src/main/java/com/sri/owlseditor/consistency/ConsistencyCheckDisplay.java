...
public class ConsistencyCheckDisplay implements CleanerListener {
	...
	private ConsistencyCheckDisplay() {
		...
		JScrollPane ccScroll = new JScrollPane(m_panel,
	...
	public void updateDisplay(Vector vData) {
		...
		for (int i = 0; i < vData.size(); i++) {
			...
			try {
				...
				ActionListener ignoreListener = (ActionListener) vEntry
						.elementAt(3);
				...
				for (int j = 0; j < vSolution.size(); j++) {
					...
					addRowToDisplay(probDoc, solDoc, lst,
							(j == 0) ? ignoreListener : null);
				}
	...
	private void addRowToDisplay(DefaultStyledDocument problemDoc,
			DefaultStyledDocument solutionDoc, ActionListener lst,
			ActionListener ignoreLst) {
		...
		int problemRowCount = Math.max(2,
				getLineCount(problemBox, paneWidth, fm));
		int solutionRowCount = Math.max(2,
				getLineCount(solutionBox, paneWidth, fm));
		int paneHeight = rowHeight
				* Math.max(problemRowCount, solutionRowCount) + 4;
	...
	public void removeInconsistencyFromDisplay(ActionListener lst_rem,
			int ignoreFlag) {
		int indx = -1;
	...
	/*
	 * should work for JTextPane but doesn't!!! private int getLineCount
	 * (JTextPane jtp) { int nlines = 0; View view = jtp.getUI().getRootView
	 * (jtp).getView(0); for (int i = 0; i < view.getViewCount(); i++) nlines +=
	 * view.getView(i).getViewCount();
	 * 
	 * return nlines; }
	 */
