 ...
 */
public class SWRLWidget extends AbstractPropertyWidget implements
		OWLWidgetMetadata {
	...
	public void setValues(Collection values) {
		// SWRLAtomList atomList =
		// (SWRLAtomList)CollectionUtilities.getFirstItem(values);
		RDFResource atomList = (RDFResource) getEditedResource()
				.getPropertyValue(getRDFProperty());
	...
	/**
	 * Deletes an AtomList and all its sub-elements. This is static because it
	 * also needs to be called from other places where atomlists need to be
	 * deleted (e.g. when a Condition is deleted).
	 */
	public static void deleteAtomList(SWRLAtomList atomList, OWLModel model) {
		...
		OWLNamedClass parameterCls = model
				.getOWLNamedClass("process:Parameter");
		...
		for (Iterator it = ownInstances.iterator(); it.hasNext();) {
			...
			if (o instanceof SWRLIndividual) {
				...
				if (!swrlIndividual.isDeleted()) {
					Iterator references = model.getTripleStoreModel()
							.getActiveTripleStore()
							.listTriplesWithObject(swrlIndividual);
					...
					if (!hasExternalRef && // !swrlIndividual.equals(atomList)
						...
						// We don't want to delete it if it is a
						// process:Parameter,
	...
	public Collection getValues() {
		...
		SWRLAtomList oldAtomList = (SWRLAtomList) oldAtomLists
	...
	// TODO: This widget shows up on ALL expressionObject properties, not just
	// SWRL
	// ones.
	public int getSuitability(RDFSNamedClass cls, RDFProperty property) {
		// Our imports mat not be loaded when initialize() is called, so we have
		// to do this here.
		...
		if (cls.hasRDFType(swrlExpression)
				&& property.isSubpropertyOf(expressionObject, true))
			return DEFAULT + 1;
		else
			return NOT_SUITABLE;
	}
