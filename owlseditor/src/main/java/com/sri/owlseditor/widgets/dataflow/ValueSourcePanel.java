...
public class ValueSourcePanel extends AbstractBindingPanel {
	...
	public ValueSourcePanel(OWLModel okb) {
		...
		fromParameterLabelPane.add(ComponentFactory
				.createLabel("From Parameter"));
	...
	/*
	 * Helper class to set valueSource. Since only one of the properties
	 * valueSource, valueForm, valueFunction, valueData or valueType is allowed
	 * at any time, we have to look for and remove all the other ones.
	 */
	private void setValueSource(Object value) {
		...
		// System.out.println("setValueSource: Added value " +
		// ((Instance)value).getName());
	}

	/* Helper method for setValueSource */
	private void removeValueSpecifier(OWLProperty valueSpecifier) {
		OWLIndividual oldValueSpecifier = (OWLIndividual) currentBinding
				.getPropertyValue(valueSpecifier);
		if (oldValueSpecifier != null) {
			currentBinding.removePropertyValue(valueSpecifier,
					oldValueSpecifier);
		}
	}

	/*
	 * Called when the Perform selection changes. We need to update the combo
	 * boxes.
	 */
	public void setPerform(OWLIndividual perform) {
		// add all parent Performs to the fromProcessChooser
		// System.out.println("ValueSourcePanel: setPerform " +
		// perform.getName());
		...
		while (it.hasNext()) {
			...
			// System.out.println("ValueSourcePanel.setPerform(): Adding predecessor Perform "
			// + pred.getName());
		}
	}
	...
	public void setBinding(OWLIndividual binding, OWLIndividual perform) {
		// System.out.println("setBinding called with perform " + perform +
		// " and binding " + binding);
		...
		if (binding != null) {
			...
			RDFIndividual fromParameter = (RDFIndividual) valueOf
					.getPropertyValue(theVarSlot);

			if (fromProcess != null) {
				// System.out.println("setBinding: selecting fromProcess: " +
				// fromProcess.getName());
				...
				if (fromProcessChooser.getItemCount() > 0) {
					fromProcess = okb
							.getOWLIndividual((String) fromProcessChooser
									.getItemAt(0));
			...
			if (fromParameter != null) {
				// System.out.println("setBinding: selecting fromParameter: " +
				...
				if (fromParameterChooser.getItemCount() > 0) {
					fromParameter = okb
							.getOWLIndividual((String) fromParameterChooser
									.getItemAt(0));
					fromParameterChooser.setSelectedIndex(0);
					valueOf.addPropertyValue(theVarSlot, fromParameter);
				}
			}
			// System.out.println("setBinding setting currentFromPerform to " +
			// currentFromPerform);
		}
		enableFromProcessListener();
		enableFromParameterListener();
	...
	/*
	 * Returns all Performs that occur previous to this one. These are the ones
	 * we can create data flow bindings for.
	 */
	private Collection getPredecessorPerforms(OWLIndividual perform) {
		Collection preds = PerformTreeMapper.getInstance().getPredecessors(
				perform);
	...
	private OWLIndividual getParentProcess() {
		OWLIndividual parent = PerformTreeMapper.getInstance()
				.getCompositeProcess(currentPerform);
		// System.out.println("The Parent composite process of this perform is: "
		// + parent);
		return parent;
	}
	...
	private OWLIndividual getThisProcess() {
		OWLIndividual thisProcess = (OWLIndividual) currentPerform
				.getPropertyValue(processSlot);
	 ...
	 * 
	 * Returns a List of all Outputs of the process that this Perform performs.
	 */
	private List getParameters(OWLIndividual fromPerform) {
		...
		if (fromProcess == null)
			...
			if (fromPerform.getName().equals("process:TheParentPerform")) {
				// return inputs
				return (List) fromProcess
						.getPropertyValues(hasInputSlot, false);
			} else {
				// return outputs
				return (List) fromProcess.getPropertyValues(hasOutputSlot,
						false);
			}
		}
	}
	...
	/*
	 * Called when the current binding is deleted. Deletes the ValueOf of this
	 * binding.
	 */
	...
	private void fromPerformSelected(OWLIndividual fromPerform) {
		...
		OWLIndividual valueOf = (OWLIndividual) currentBinding
				.getPropertyValue(valueSourceSlot);
		if (valueOf == null) {
			valueOf = valueOfCls.createOWLIndividual(null);
		}

		OWLIndividual oldValue = (OWLIndividual) valueOf
				.getPropertyValue(fromProcessSlot);
		if (oldValue != null)
	...
	public void fromParameterSelected(RDFIndividual fromParameter) {
		...
		RDFIndividual oldValue = (OWLIndividual) valueOf
				.getPropertyValue(theVarSlot);
		if (oldValue != null) {
			valueOf.removePropertyValue(theVarSlot, oldValue);
		}
		...
		if (fromProcess == null)
			System.out
					.println("WARNING! fromParameterSelected setting ValueOf with NULL fromProcess!");

		setValueSource(valueOf);
	}

	/*
	 * This listener makes sure that the parameter chooser gets updated when the
	 * selections of Perform changes, so that only the parameters of the
	 ...
	 * diableFromProcessListener() before doing any programmatic changes to the
	 * fromProcessChooser.
	 */
