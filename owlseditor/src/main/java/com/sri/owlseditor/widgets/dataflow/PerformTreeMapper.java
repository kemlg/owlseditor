...
/**
 * This singleton class maintains a mapping from each Perform and Produce to the
 * composite process that the Perform/Produce is part of, and to a list of the
 * Perform's/Produce's predecessors in the composition tree.
 * 
 * This is used by the HasDataFromWidget and ProducedBindingWidget. Only these
 * predecessors are shown in the "from perform" combo box, since a Perform can
 * only bind inputs from the outputs of (unconditional) predecessors in the
 * composition graph.
 * 
 * Also, this mapping is used by the HasDataFromWidget to decode
 * TheParentPerform of a perform.
 * 
 * The mappings are built up in the graph() methods of the OWLSTreeNodes. This
 * means that the widgets that depend on this will not work correctly before the
 * graph has been drawn. But they probably don't need to either.
 * 
 * 
...
public class PerformTreeMapper implements CleanerListener {
	...
	public void put(OWLIndividual perform, OWLIndividual composite) {
		if (map.containsKey(perform)) {
			// System.out.println("WARNING! PerformTreeMapper already contains key "
			// + perform);
		} else {
			// System.out.println("PerformTreeMapper: Adding cell for instance "
			// + perform.getName() +
	...
	public OWLIndividual getPredecessorPerformWithProcess(
			OWLIndividual perform, OWLIndividual process) {
		...
		OWLObjectProperty processProperty = model
		...
		while (it.hasNext()) {
			...
			OWLIndividual predProcess = (OWLIndividual) predPerform
	...
	public void addPredecessors(OWLIndividual perform, Set preds) {
		MapCell cell = (MapCell) map.get(perform);
		// System.out.println("PerformTreeMapper.addPredecessors() adding " +
		// preds);
	...
	/**
	 * Regenerates the predecessors list for all the Performs. If the Perform is
	 * not in the PerformTreeMapper (i.e. it is newly created) it will be added
	 * to it.
	 * 
	 * @param node
	 * @param cp
	 *            The composite process that this perform is a part of
	 */
	public void generatePredecessors(OWLSTreeNode root, OWLIndividual perform,
			OWLIndividual cp) {
	 ...
	 * We can't just look for the newly created/moved one, because moving one
	 * perform causes changes in the predecessors of others.
	 */
	private void regeneratePreds(OWLSTreeNode current, Set preds) {
		...
		if (current instanceof ProduceNode) {
			...
			OWLIndividual theParentPerform = okb
					.getOWLIndividual("process:TheParentPerform");
		...
		// This may need to be handled better (marking the nodes somehow).
			...
			if (current.getChildCount() > 0) {
				...
				if (thennode != null) {
					...
					if (current.getChildCount() > 1) {
						OWLSTreeNode elsenode = (OWLSTreeNode) current
								.getChildAt(1);
						if (elsenode != null) {
			...
			System.out.println("Unsupported control construct on node "
					+ current);
		}
	}
	...
	private void removeIllegalInputBindings(OWLIndividual perform) {
		...
		Collection inputBindings = perform.getPropertyValues(hasDataFromSlot,
				true);
	...
	private void removeIllegalOutputBindings(OWLIndividual perform) {
		OWLModel okb = (OWLModel) perform.getOWLModel();
		OWLProperty producedBindingSlot = okb
				.getOWLProperty("process:producedBinding");
		OWLProperty valueSourceSlot = okb.getOWLProperty("process:valueSource");
		OWLProperty fromProcessSlot = okb.getOWLProperty("process:fromProcess");
		OWLProperty theVarSlot = okb.getOWLProperty("process:theVar");

		Collection outputBindings = perform.getPropertyValues(
				producedBindingSlot, true);
		Iterator it = outputBindings.iterator();
		while (it.hasNext()) {
			OWLIndividual outputBinding = (OWLIndividual) it.next();
			OWLIndividual valueOf = (OWLIndividual) outputBinding
					.getPropertyValue(valueSourceSlot);
			OWLIndividual fromProcess = (OWLIndividual) valueOf
					.getPropertyValue(fromProcessSlot);
			OWLIndividual theVar = (OWLIndividual) valueOf
...
class MapCell {
	...
	public void addPredecessor(OWLIndividual pred) {
		// System.out.println("MapCell.addPredecessor() adding predecessor " +
		// pred.getName());
		preds.add(pred);
		// System.out.println("MapCell.addPredecessor() predecessors are now " +
		// preds);
	}

	public void addPredecessors(Set preds) {
		// System.out.println("MapCell.addPredecessors() adding predecessors " +
		// preds);
