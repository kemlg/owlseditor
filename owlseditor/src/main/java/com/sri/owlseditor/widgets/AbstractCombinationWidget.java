...
/**
 * This is a superclass of HasDataFromWidget and WsdlMessageMapWidget, which
...
public abstract class AbstractCombinationWidget extends AbstractPropertyWidget {
	...
	private JPanel editorContainer; // The contents of this changes depending on
									// selectionCombo selection
	...
	/**
	 * Note: This method is responsible for updating the combo box! This only
	 * gets called when an item is selected, not when items are unselected. When
	 * that happens, i.e. when no list item is selected, the editor field gets
	 * emptied.
	 */
	public abstract void listSelectionChanged(Object selectedItem);

	/**
	 * The component of the editorContainer will already be changed when this is
	 * called.
	 * 
	...
	/**
	 * @param parent
	 *            The parent component is passed so that implementors can pop up
	 *            a pick dialog.
	 * @return The object that should be added to the list. Null if no object.
	 *         This item will become selected if it is non-null.
	 */
	 ...
	 * @param comboLabel
	 * @param clsesMap
	 *            Keys should be the strings (or other Objects) to appear in the
	 *            combo box, and values should be the JPanels associated with
	 *            those labels.
	 */
	public void initialize(String listLabel, String addToolTip,
			String removeToolTip, String comboLabel, HashMap clsesMap) {
		...
		selectionComboListener = new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				if (e.getStateChange() == ItemEvent.SELECTED) {
					JPanel comp = (JPanel) getClsesMap().get(
							getComboSelection());
		...
		rightSidePanel
				.setLayout(new BoxLayout(rightSidePanel, BoxLayout.Y_AXIS));
		...
		rsp_gbc.gridx = GridBagConstraints.RELATIVE; // just to the right of the
														// previous component
	...
	public void setEditorComponent(JPanel component) {
		// System.out.println("setEditorComponent(): Changing component to " +
		// component);
	...
	private LabeledComponent createLabeledComponent(String listLabel,
			String addToolTip, String removeToolTip) {
		LabeledComponent lc = ComponentFactory.createLabeledScrollComponent(
				...
				new Dimension(ComponentUtilities.getStandardColumnWidth() * 2,
						ComponentUtilities.getStandardRowHeight() * 3), null,
				null, null);
		...
		addAction.putValue(Action.SMALL_ICON,
		...
		class RemoveAction extends AllowableAction {
			...
			public RemoveAction(Component parent, String name,
