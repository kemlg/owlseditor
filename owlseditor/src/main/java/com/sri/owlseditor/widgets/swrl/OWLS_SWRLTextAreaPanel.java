 ...
 * 
 * Changes: 1) A setImp() method 2) Uses a modified toolbar that doesn't have
 * the Imp button. 3) Added change listeners, and a constructor parameter to
 * pass the slot widget, so that we can call valueChanged() when there is
 * editing. 4) A method to remove those listeners.
 * 
 * @author Holger Knublauch <holger@smi.stanford.edu>
 * @author Daniel Elenius <elenius@csl.sri.com>
 */
public class OWLS_SWRLTextAreaPanel extends JPanel implements
	...
	public OWLS_SWRLTextAreaPanel(OWLModel okb, SWRLAtomList atomList,
			SWRLWidget swrlWidget) {
		this.okb = okb;
		...
		textArea = new SWRLTextArea(okb, symbolPanel) {
			protected void checkExpression(String text) throws Throwable {
				(new CompactOWLClassParser()).checkClass(
						OWLS_SWRLTextAreaPanel.this.okb, text);
			}
		};
	...
	public static boolean showEditDialog(Component parent, OWLModel okb,
			SWRLImp imp) {
		SWRLTextAreaPanel panel = new SWRLTextAreaPanel(okb, imp);
		String title = "Edit SWRL Rule";
		if (ModalDialog.showDialog(parent, panel, title,
				ModalDialog.MODE_OK_CANCEL) == ModalDialog.OPTION_OK) {
			try {
