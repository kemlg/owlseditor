...
public class GenerateFromWSDLAction extends AbstractAction {
	 ...
	 * private JDialog parentFrame; private int choiceNumber = 1; public
	 * ChoicesPanel(JDialog theParent) { super(); choiceNumber = 1; parentFrame
	 * = theParent; JButton nextButton = new JButton("Next...");
	 * nextButton.addActionListener(new ActionListener() { public void
	 * actionPerformed(ActionEvent e) { parentFrame.dispose(); } }); JButton
	 * cancelButton = new JButton("Cancel"); cancelButton.addActionListener(new
	 * ActionListener() { public void actionPerformed(ActionEvent e) {
	 * choiceNumber = 0; parentFrame.dispose(); } }); JRadioButton buildRadio =
	 * new JRadioButton("Build new project and import the WSDL file",true);
	 * JRadioButton importRadio = new
	 * JRadioButton("Import the WSDL file to the current project");
	 * buildRadio.setActionCommand("build");
	 * importRadio.setActionCommand("import"); ButtonGroup theChoicesGroup = new
	 * ButtonGroup(); theChoicesGroup.add(buildRadio);
	 * theChoicesGroup.add(importRadio); buildRadio.addActionListener(this);
	 * importRadio.addActionListener(this); //Put the radio buttons in a column
	 * in a panel. JPanel radioPanel = new JPanel(new GridLayout(0, 1));
	 * radioPanel.add(buildRadio); radioPanel.add(importRadio); add(radioPanel,
	 * BorderLayout.LINE_START); //add(picture, BorderLayout.CENTER);
	 * setBorder(BorderFactory.createEmptyBorder(20,20,20,20));
	 * this.add(nextButton); this.add(cancelButton); } public void
	 * actionPerformed(ActionEvent e) { if ( e.getActionCommand() == "build" )
	 * choiceNumber = 1; else if ( e.getActionCommand() == "import" )
	 * choiceNumber = 2; } public int getChoiceNumber() { return choiceNumber; }
	 * }
	 */

	public void actionPerformed(ActionEvent e) {
		wsdlImport = new JDialog((java.awt.Frame) Application.getMainWindow(),
				"Import a WSDL File", true);
		...
		if (wsdl2owls.isSuccessful()) {
			...
			if (current != top) {
				...
				GenerateFromWSDLAction.importOWLFile(
						wsdl2owls.getOwlFileName(), okb, "wi", defNameSpace);
	...
	static public boolean importOWLFile(String filename, OWLModel okb,
			String pref, String dns) {
