...
/** This uses StringListWidget, which is in Protege-core rather than Protege-OWL, but
 * there's nothing we can do about that right now.
 */
public class ParamTypeWidget extends AbstractListWidget implements OWLWidgetMetadata {
    ...
    public int getSuitability(RDFSNamedClass cls, RDFProperty property){
		if (property.getName().equals("process:parameterType")) 
    ...
    /* This method traps calls to addItem in the parent class, and first adds a listener
     * for class delete events.
     */
    public void addItem(Object o){
    	if (o instanceof RDFSClass){
    		RDFSClass cls = (RDFSClass)o;
    		ModelAdapter listener = new ModelAdapter(){
    			OWLDatatypeProperty parameterType = m_okb.getOWLDatatypeProperty("process:parameterType");

    			public void classDeleted(RDFSClass cls){
    				((RDFResource)getInstance()).removePropertyValue(parameterType, cls.getURI());
    			}
    		};
    ...
    private Action getCreateClassAction() {
        _createAction = new CreateAction ("Add OWL Class", OWLIcons.getAddIcon(OWLIcons.PRIMITIVE_OWL_CLASS)) {
    ...
    private Action getCreateXSDAction() {
        _createAction = new CreateAction ("Add XSD Datatype", OWLIcons.getAddIcon(OWLIcons.RDF_DATATYPE)) {
            public void onCreate() {
	...
	Collection c = CollectionUtilities.createCollection (m_okb.getOWLThingClass());
        OWLNamedClass cls = (OWLNamedClass)DisplayUtilities.pickCls ((Component) ParamTypeWidget.this, m_okb, c, "Select Cls");
     ...
     protected void handleCreateXSDAction() {
     	//RDFSNamedClass datatype = m_okb.getRDFSNamedClass(RDFSNames.Cls.DATATYPE);
     	//Collection datatypes = sortDatatypes(datatype.getInstances(true));
     	RDFSDatatype type = (RDFSDatatype) DisplayUtilities.pickInstanceFromCollection(this, 
     																				   getSortedDatatypes(), 
																					   0,
     																				   "Pick a datatype");
	...
	while (iter.hasNext()) {
		...
		_list.setSelectedIndex (Math.min (indx, _list.getModel().getSize() - 1));
	    }
	}
	//removeItems (strings);
    }
   ...
   // return the current value displayed by the widget
	...
	for (int i = 0; i < _list.getModel().getSize(); i++) {
	    ...
	    DefaultRDFSLiteral literal = (DefaultRDFSLiteral)m_okb.createRDFSLiteral(item.getURI(), 
	    											     						 m_okb.getRDFSDatatypeByURI(XSDDatatype.XSDanyURI.getURI()));
	    //System.out.println("ParamTypeWidget.getValues(): Type of literal is " + m_okb.getRDFSDatatypeOfValue(literal));
	    //System.out.println("getValues() returning " + literal.getRawValue());
	    coll.add (literal);
	}
	return coll;
    }
    ...
    public void setValues (Collection c)
    {
    	...
    	while (iter.hasNext()) {
		    RDFSLiteral literal = (RDFSLiteral)iter.next();
		    //System.out.println("parameterType.setValues() got a " + o.getClass() + " with value " + o);
		    //if (o instanceof RDFSLiteral)
		    //	System.out.println("raw value: " + ((DefaultRDFSLiteral)o).getRawValue());
			
		    //System.out.println("ParamTypeWidget.setValues(): Type of literal is " + (Object)m_okb.getRDFSDatatypeOfValue(o));
