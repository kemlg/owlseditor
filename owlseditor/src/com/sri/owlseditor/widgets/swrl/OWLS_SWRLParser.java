...
public class OWLS_SWRLParser {
    ...
    public SWRLAtomList parse(String rule, SWRLAtomList atomList) throws SWRLParseException {
    	String token;
    ...
    throws SWRLParseException {
		...
		if (parsedString.equals("?")){
			...
			if (!parseOnly)
				parsedEntity = owlModel.createRDFSLiteral(stringValue, owlModel.getXSDstring());  
			getNextNonSpaceToken("Expected \"");
		}
		else if (parsedString.equals("true") || parsedString.equals("false")){
			// The parsed entity is a boolean
			if (!parseOnly)
				parsedEntity = owlModel.createRDFSLiteral(parsedString, owlModel.getXSDboolean());  
		}
		...
		// parsed as xsd:int's.
			...
			if (!parseOnly)
				parsedEntity = owlModel.createRDFSLiteral("true", owlModel.getXSDboolean());  
		}
		else if (parsedString.equals("False")){
			if (!parseOnly)
				parsedEntity = owlModel.createRDFSLiteral("false", owlModel.getXSDboolean());  
		}
		else{
    ...
    private SWRLVariable getSWRLVariable(String name) throws SWRLParseException {
        ...
        if (resource instanceof SWRLVariable) {
            System.out.println("returning existing resource " + resource.getName());
        		...
        		OWLNamedClass swrlVarCls = owlModel.getOWLNamedClass("swrl:Variable");
