...
public class ExprBodyWidget extends TextAreaWidget implements OWLWidgetMetadata {
    ...
    public Collection getValues()
    {
	    ...
	    DefaultRDFSLiteral literal = (DefaultRDFSLiteral)m_okb.createRDFSLiteral(str, m_okb.getRDFXMLLiteralType());
	    //System.out.println("ExpressionBodyWidget: Literal raw value is: " + literal.getRawValue());
    ...
    public void setWidgetValues() {
        Collection values;
        OWLIndividual expression = (OWLIndividual)getInstance();
        OWLDatatypeProperty exprbody = (OWLDatatypeProperty)getSlot();
        values = new ArrayList(expression.getPropertyValues(exprbody, true));
        setValues(values);		
    }
    */
