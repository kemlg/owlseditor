...
public class WSDLOutputWidget extends AbstractCombinationWidget implements OWLWidgetMetadata {
	...
	super.initialize ("WSDL Outputs", "Add", "Remove", "Wsdl Options", createClsesMap());
	...
	wsdlOutputMessageMapCls = m_okb.getOWLNamedClass ("grounding:WsdlOutputMessageMap");
	processOutputCls        = m_okb.getOWLNamedClass ("process:Output");

    wsdlOutputSlot               = (OWLObjectProperty) m_okb.getOWLProperty ("grounding:wsdlOutput");
	...
	wsdlMsgEditor.getTextArea().addFocusListener (new WSDLMsgFocusListener());

	wsdlXSLTButton = new WSDLXSLTButtonPanel();

	wsdlURIEditor = new WSDLStringPanel();
        wsdlURIEditor.getTextArea().addFocusListener (new WSDLURIFocusListener());
	...
	ActionListener wsdlXSLTButtonListener = new ActionListener() {
	    ...
	    DefaultRDFSLiteral l = (DefaultRDFSLiteral)m_okb.createRDFSLiteral(xsltString, m_okb.getRDFXMLLiteralType());
	    updateKB (owlInst, XSLT_STR, l);
		*/
		...
		public void actionPerformed (ActionEvent evt) {
		    ...
		    OWLDatatypeProperty wsdlOutputMessageProp = (OWLDatatypeProperty) m_okb.getOWLProperty ("grounding:wsdlOutputMessage");
		    RDFSLiteral wsdlOutputMessage = (RDFSLiteral) m_inst.getPropertyValue (wsdlOutputMessageProp);
		    ...
		    OWLIndividual outputMessageMap = (OWLIndividual) m_msgMap.get (m_selItem);
		    try {
			W2OXSLTDialog dlg = new W2OXSLTDialog (m_okb, atomicProcess, wsdlDocument.toString(), wsdlOutputMessage.toString(), outputMessageMap);
			dlg.show();
    ...
    */
	...
	/*
	if (propVal == null || strID.equals(MSG_PART) || strID.equals(XSLT_URI)) {
	    wsdlStrEditor.getTextArea().setText("");
	...
	*/
	...
	RDFSLiteral lit = (RDFSLiteral) owlInst.getPropertyValue (wsdlMessagePartSlot);
	if (lit != null)
    ...
    {
	RDFSLiteral msgPart = (RDFSLiteral) owlInst.getPropertyValue (wsdlMessagePartSlot);
	if (msgPart != null)
	    return MSG_PART;
	
	RDFSLiteral xsltStr = (RDFSLiteral) owlInst.getPropertyValue (xsltTransformationStringSlot);
	if (xsltStr != null)
	    return XSLT_STR;

	RDFSLiteral xsltURI = (RDFSLiteral) owlInst.getPropertyValue (xsltTransformationURISlot);
	if (xsltURI != null)
	    return XSLT_URI;
    ...
    RDFIndividual inst = (RDFIndividual) DisplayUtilities.pickInstance ((Component) WSDLOutputWidget.this, c);
	if (m_msgMap.containsKey (inst))
	    return null;

	OWLIndividual wsdlInst = (OWLIndividual) wsdlOutputMessageMapCls.createInstance (null);
	...
	if (strID.equals (MSG_PART)) {
	    RDFSLiteral l = (RDFSLiteral)owlInst.getPropertyValue (wsdlMessagePartSlot);
		if (l != null)
			wsdlMsgEditor.getTextArea().setText (l.getString());
	}
	else if (strID.equals (XSLT_STR)) {
	    /*
	    RDFSLiteral l = (RDFSLiteral)owlInst.getPropertyValue (xsltTransformationStringSlot);
		if (l != null)
			wsdlStrEditor.getTextArea().setText (l.getString());
	    */
	}
	else if (strID.equals (XSLT_URI)) {
	    RDFSLiteral l = (RDFSLiteral)owlInst.getPropertyValue (xsltTransformationURISlot);
		if (l != null)
	...
	public void focusLost (FocusEvent event) {
	    ...
	    DefaultRDFSLiteral l = (DefaultRDFSLiteral)m_okb.
								createRDFSLiteral(mpartString, m_okb.getRDFSDatatypeByName("xsd:anyURI"));
	...
	public void focusLost (FocusEvent event) {
	    ...
	    DefaultRDFSLiteral l = (DefaultRDFSLiteral)m_okb.
								createRDFSLiteral(xslturiString, m_okb.getRDFSDatatypeByName("xsd:anyURI"));
