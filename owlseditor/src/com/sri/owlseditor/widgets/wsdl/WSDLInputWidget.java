...
public class WSDLInputWidget extends AbstractCombinationWidget implements OWLWidgetMetadata {
	...
	super.initialize ("WSDL Inputs", "Add", "Remove", "Wsdl Options", createClsesMap());
	...
	wsdlInputMessageMapCls = m_okb.getOWLNamedClass ("grounding:WsdlInputMessageMap");
	processInputCls        = m_okb.getOWLNamedClass ("process:Input");

    wsdlInputSlot                = (OWLObjectProperty) m_okb.getOWLProperty ("grounding:wsdlInput");
	wsdlMessagePartSlot          = (OWLDatatypeProperty) m_okb.getOWLProperty ("grounding:wsdlMessagePart");
	wsdlOwlSParameterSlot        = (OWLObjectProperty) m_okb.getOWLProperty ("grounding:owlsParameter");
	xsltTransformationStringSlot = (OWLDatatypeProperty) m_okb.getOWLProperty ("grounding:xsltTransformationString");
	xsltTransformationURISlot    = (OWLDatatypeProperty) m_okb.getOWLProperty ("grounding:xsltTransformationURI");
    }

    private void showWSDLErrorDialog(String message){
    	JOptionPane.showMessageDialog(this, message, "WSDL Error!", JOptionPane.ERROR_MESSAGE);
    }
	...
	wsdlXSLTButton = new WSDLXSLTButtonPanel();
        //wsdlXSLTButton.getXSLTButton().addActionListener (wsdlXSLTButtonListener);
	wsdlStringEditor = new WSDLStringPanel();
        wsdlStringEditor.getTextArea().addFocusListener (new WSDLURIFocusListener());
	wsdlOwlSParamEditor = new WSDLOwlSParamPanel();
	
	wsdlOwlSParamEditor.getOwlSParamWidget().addAddHeaderButton (getAddAction());
	wsdlOwlSParamEditor.getOwlSParamWidget().addRemoveHeaderButton (getRemoveAction());
	...
	ActionListener wsdlXSLTButtonListener = new ActionListener() {
		public void actionPerformed (ActionEvent evt) {
		    OWLDatatypeProperty wsdlDocumentProp = (OWLDatatypeProperty) m_okb.getOWLProperty ("grounding:wsdlDocument");
		    RDFSLiteral wsdlDocument = (RDFSLiteral) m_inst.getPropertyValue (wsdlDocumentProp);

		    OWLDatatypeProperty wsdlInputMessageProp = (OWLDatatypeProperty) m_okb.getOWLProperty ("grounding:wsdlInputMessage");
		    RDFSLiteral wsdlInputMessage = (RDFSLiteral) m_inst.getPropertyValue (wsdlInputMessageProp);

		    OWLObjectProperty owlsProcessProp= (OWLObjectProperty) m_okb.getOWLProperty ("grounding:owlsProcess");
		    OWLIndividual atomicProcess = (OWLIndividual) getEditedResource().getPropertyValue (owlsProcessProp);

		    /*
		    OWLDatatypeProperty wsdlMessagePartProp = (OWLDatatypeProperty) m_okb.getOWLProperty ("grounding:wsdlMessagePart");
		    RDFSLiteral wsdlMessagePart = (RDFSLiteral) m_inst.getPropertyValue (wsdlMessagePartProp);
		    */

		    OWLIndividual inputMessageMap = (OWLIndividual) m_msgMap.get (m_selItem);
		    ...
		    try{
		    	dlg = new O2WXSLTDialog (m_okb, atomicProcess, wsdlDocumentString, wsdlInputMessageString, inputMessageMap);
			    dlg.show();
		    }
     ...
     * If we get AbstractSlotWidget.setWidgetValues - ClassCastException,
     * uncomment this.
    public void setWidgetValues() {
        Collection values;
        OWLIndividual grounding = (OWLIndividual)getInstance();
        OWLObjectProperty map = (OWLObjectProperty)getSlot();
        values = new ArrayList(grounding.getPropertyValues(map, true));
        setValues(values);		
    }
    */
	...
	while (it.hasNext()) {
	    ...
	    RDFSLiteral l = (RDFSLiteral)inst.getPropertyValue(wsdlMessagePartSlot);
	...
	if (propVal != null) {
		...
		wsdlOwlSParamEditor.getOwlSParamWidget().enableRemoveButton (true);
	    }
	    /*
	    else if (strID.equals (XSLT_STR)) {
		owlInst.setPropertyValue (xsltTransformationStringSlot, propVal);
	    }
	    */
	...
	/*
	if (propVal == null || strID.equals(OWLS_PARAM) || strID.equals(XSLT_URI)) {
	    wsdlTextEditor.getTextArea().setText("");
	    wsdlTextEditor.getTextArea().repaint();
	}
	*/
	if (propVal == null || strID.equals(OWLS_PARAM) || strID.equals(XSLT_STR)) {
	...
	RDFIndividual owlSParam = (RDFIndividual) owlInst.getPropertyValue (wsdlOwlSParameterSlot);
	if (owlSParam != null)
	    owlInst.removePropertyValue (wsdlOwlSParameterSlot, owlSParam);

	String xsltStr = (String) owlInst.getPropertyValue (xsltTransformationStringSlot);
	if (xsltStr != null)
	    owlInst.removePropertyValue (xsltTransformationStringSlot, xsltStr);

	String xsltURI = (String) owlInst.getPropertyValue (xsltTransformationURISlot);
	if (xsltURI != null)
	...
	String s = DisplayUtilities.editString (parent, "Add URI String", null, null);
	DefaultRDFSLiteral l = (DefaultRDFSLiteral)m_okb.
								createRDFSLiteral(s, m_okb.getRDFSDatatypeByName("xsd:anyURI"));

	if (m_msgMap.containsKey (l))
	    return null;

	OWLIndividual wsdlInst = (OWLIndividual) wsdlInputMessageMapCls.createInstance (null);
	...
	OWLIndividual owlInst = (OWLIndividual) m_msgMap.get (getSelectedListValue());
	...
	if (strID.equals (OWLS_PARAM)) {
	    JList jl = wsdlOwlSParamEditor.getOwlSParamList();
	    Collection ci = CollectionUtilities.createCollection (owlInst.getPropertyValue (wsdlOwlSParameterSlot));
	    ComponentUtilities.setListValues (jl, ci);
	    wsdlOwlSParamEditor.getOwlSParamWidget().enableRemoveButton (ci != null);
	}
	else if (strID.equals (XSLT_STR)) {
	    //wsdlTextEditor.getTextArea().setText ((String) owlInst.getPropertyValue (xsltTransformationStringSlot));
	}
	else if (strID.equals (XSLT_URI)) {
	    wsdlStringEditor.getTextArea().setText ((String) owlInst.getPropertyValue (xsltTransformationURISlot));
	}
    }
	...
	Collection c = CollectionUtilities.createCollection (processInputCls);
        RDFIndividual inst = (RDFIndividual) DisplayUtilities.pickInstance ((Component) WSDLInputWidget.this, c);
        if (inst != null) {
	    JList jl = wsdlOwlSParamEditor.getOwlSParamList();
    ...
    /*
    private class WSDLXSLTButtonListener extends FocusAdapter
    {
	...
	public void focusLost (FocusEvent event) {
	    OWLIndividual owlInst = (OWLIndividual) m_msgMap.get (selectedListValue);
	    updateKB (owlInst, XSLT_STR, wsdlTextEditor.getTextArea().getText());
	}
    }
    */
	...
	public void focusLost (FocusEvent event) {
	    OWLIndividual owlInst = (OWLIndividual) m_msgMap.get (selectedListValue);
	    updateKB (owlInst, XSLT_URI, wsdlStringEditor.getTextArea().getText());
	}
    }
}
