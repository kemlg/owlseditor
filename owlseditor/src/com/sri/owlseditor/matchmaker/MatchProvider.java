...
public interface MatchProvider {
	...
	/** Returns a list of MatchResults that have outputs
	 * to match the provided process's inputs. */
	public List findInputProviders(OWLIndividual perform);

	/** Returns a list of MatchResults that have inputs
	 * to match the provided process's outputs. */
	public List findOutputConsumers(OWLIndividual perform);

	/** Returns the kb object corresponding to a certain match.
	 * If the MatchProvider is non-local, this is where it has to retrieve
	 * and import the ontology containing the process. When this method
	 * has returned, all IOs in this process and their parameterType classes 
	 * must also be in the local kb, under the same names that they use in the 
