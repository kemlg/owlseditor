...
public class OutputConsumerFinder extends MatchFinder {
	...
	private void createOutgoingDataFlow(MatchResult mResult,
			OWLIndividual oldPerform, OWLIndividual newPerform) {
		// System.out.println("Adding data flow from " + oldPerform.getName() +
		// " to " + newPerform.getName());

		OWLIndividual newProcess = (OWLIndividual) perform
		...
		while (it.hasNext()) {
			...
			if (pair != null) {
				...
				if (inputstring != null) {
					OWLIndividual input = (OWLIndividual) model
							.getRDFResource(inputstring);
					if (input != null) {
						if (newPerform.hasRDFType(performCls, true)) {
							OWLIndividual inputBinding = inputBindingCls
									.createOWLIndividual(null);
							inputBinding.setPropertyValue(toParam, input);
							OWLIndividual valueOf = valueOfCls
									.createOWLIndividual(null);
							...
							newPerform.addPropertyValue(hasDataFrom,
									inputBinding);
						} else if (newPerform.hasRDFType(produceCls, true)) {
							OWLIndividual outputBinding = outputBindingCls
									.createOWLIndividual(null);
							...
							valueOf.setPropertyValue(theVar, output);
							outputBinding
									.setPropertyValue(valueSource, valueOf);
							newPerform.addPropertyValue(producedBinding,
									outputBinding);
						}
					}
				}
	...
	/**
	 * Adds the match in a Sequence after the old perform, and updates the
	 * dataflow between the two
	 */
	private void addAfter(OWLIndividual oldPerform, MatchResult mResult) {
		...
		// System.out.println("addAfter: parent construct is " +
		// parentConstruct.getName() +
		// " of class " + parentConstruct.getRDFType().getName());

		OWLIndividual newPerform = null;
		if (newProcess == PerformTreeMapper.getInstance().getCompositeProcess(
				oldPerform)) {
		...
		if (parentConstruct == null) {
			...
			OWLIndividual sequenceConstruct = sequenceCls
					.createOWLIndividual(null);
