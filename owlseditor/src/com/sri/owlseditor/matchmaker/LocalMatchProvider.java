...
/**
 * The algorithm used here lets the same parameter be bound to/compared with
 * several parameters in the other process. Perhaps we should add an option to
...
public class LocalMatchProvider implements MatchProvider {
	...
	private void setupClassesAndProperties() {
		...
		textDescription = model
				.getOWLDatatypeProperty("profile:textDescription");
	...
	private boolean connectedToReasoner() {
		...
		if (connected) {
			...
			System.out
					.println("  LocalMatchProvider connected to DL reasoner: "
	...
	/**
	 * Goes to the service of this process, and then to all profiles of the
	 * service, until a profile:textDescription is found.
	 */
	private String getTextDescription(OWLIndividual process) {
		...
		while (it.hasNext()) {
			...
			while (it2.hasNext()) {
				...
				String text = (String) profile
						.getPropertyValue(textDescription);
				if (text != null)
					return text;
			}
		}
	...
	public List findMatchingProcesses(OWLIndividual originalPerform) {
		...
		if (connectedToReasoner()) {
			ArrayList allProcesses = new ArrayList(
					processClass.getInstances(true));
			...
			while (it.hasNext()) {
				...
				// newProcess.getName());
				results.add(getMatchingProcessMatchResult(originalProcess,
						newProcess));
			}
			return results;
		}
		System.out
				.println("  ERROR! LocalMatchProvider could not connect to reasoner.");
		return null;
	}
	...
	public List findInputProviders(OWLIndividual originalPerform) {
		...
		if (connectedToReasoner()) {
			...
			while (it.hasNext()) {
				OWLIndividual newProcess = (OWLIndividual) it.next();
				results.add(getInputProviderMatchResult(originalPerform,
						newProcess));
			}
			return results;
		}
	...
	public List findOutputConsumers(OWLIndividual originalPerform) {
		...
		if (connectedToReasoner()) {
			...
			while (it.hasNext()) {
				OWLIndividual newProcess = (OWLIndividual) it.next();
				results.add(getOutputConsumerMatchResult(originalPerform,
						newProcess));
			}
			return results;
		}
	...
	private RDFResource getParameterType(RDFResource parameter) {
		DefaultRDFSLiteral literal = (DefaultRDFSLiteral) parameter
	...
	/**
	 * Updates the MatchResult provided with the best matches in newParameters
	 * for all the originalParameters
	 */
	private void addBestMatches(MatchResult matchResult,
			Collection originalParameters, Collection newParameters) {
		...
		while (it.hasNext()) {
			...
			MatchPair bestMatch = new MatchPair(null, null, null, null,
					Matchmaker.FAIL);

			if (oldParameterType != null) {
				// System.out.println("    Looking at parameter " +
				// originalParameter.getName());

				Iterator it2 = newParameters.iterator();
				while (it2.hasNext()
						&& bestMatch.getMatchType() != Matchmaker.EQUIVALENT) {
					OWLIndividual newParameter = (OWLIndividual) it2.next();
					String newParameterName = newParameter.getName();
					// System.out.println("    Comparing with parameter " +
					// newParameterName);
					...
					if (newParameterType != null) {
						...
						if (result == Matchmaker.EQUIVALENT) {
							...
							// System.out.println("SUBSUMED match " +
							// originalParameter.getName() +
							// ", " + newParameterName);
			...
			if (input) {
				if (bestResult != Matchmaker.FAIL) {
					...
					unusedParameters.remove(model.getRDFResource(bestMatch
							.getNewParameter()));
					// System.out.println("Removed " +
					// bestMatch.getNewParameter()
					// + " from unusedParameters");
				} else
					matchResult.addInputMatch(new MatchPair(originalParameter,
							oldParameterType, null, null, Matchmaker.FAIL));
			} else {
				if (bestResult != Matchmaker.FAIL) {
		...
		if (input) {
			while (it.hasNext()) {
				...
				if (type != null)
					matchResult.addExtraInput(newInput.getName(),
							type.getName());
				else
			...
			while (it.hasNext()) {
				...
				if (type != null)
					matchResult.addExtraOutput(newOutput.getName(),
							type.getName());
				else
	...
	private MatchResult getMatchingProcessMatchResult(
			OWLIndividual originalProcess, OWLIndividual newProcess) {
		MatchResult matchResult = new MatchResult(originalProcess,
				newProcess.getName(), this);

		Collection originalInputs = originalProcess
				.getPropertyValues(inputProperty);
		Collection newInputs = newProcess.getPropertyValues(inputProperty);
		addBestMatches(matchResult, originalInputs, newInputs);

		Collection originalOutputs = originalProcess
				.getPropertyValues(outputProperty);
	...
	/**
	 * This is very similar to getMatchingProcessMatchResult, but for this one,
	 * we 1) don't care about the outputs of the original process, and 2 compare
	 * the inputs of the original with the outputs of the new process
	 * 
	 * @param originalProcess
	 * @param newProcess
	 * @return
	 */
	private MatchResult getInputProviderMatchResult(
			OWLIndividual originalPerform, OWLIndividual newProcess) {
		OWLIndividual originalProcess = (OWLIndividual) originalPerform
				.getPropertyValue(processProperty);
		OWLIndividual parentProcess = PerformTreeMapper.getInstance()
				.getCompositeProcess(originalPerform);
		...
		if (newProcess == parentProcess) {
			Collection originalInputs = originalProcess
					.getPropertyValues(inputProperty);
			Collection parentInputs = newProcess
					.getPropertyValues(inputProperty);
			addBestMatches(matchResult, originalInputs, parentInputs);
		} else {
			Collection originalInputs = originalProcess
					.getPropertyValues(inputProperty);
			Collection newOutputs = newProcess
					.getPropertyValues(outputProperty);
	...
	private MatchResult getOutputConsumerMatchResult(
			OWLIndividual originalPerform, OWLIndividual newProcess) {
		OWLIndividual originalProcess = (OWLIndividual) originalPerform
				.getPropertyValue(processProperty);
		OWLIndividual parentProcess = PerformTreeMapper.getInstance()
		...
		if (newProcess == parentProcess) {
			Collection originalOutputs = originalProcess
					.getPropertyValues(outputProperty);
			Collection parentOutputs = newProcess
					.getPropertyValues(outputProperty);
	...
	private int compare(RDFResource oldType, RDFResource newType) {
		if (oldType instanceof OWLNamedClass
			...
			try {
				// New version
				int relationship = reasoner.getSubsumptionRelationship(
						oldClass, newClass, listener);

				// We use our own constants for more generality. For example, we
				// may
				...
				/*
				 * -- old version subsumed = reasoner.isSubsumedBy(oldClass,
				 * newClass, listener); subsumes =
				 * reasoner.isSubsumedBy(newClass, oldClass, listener);
				 * 
				 * if (subsumed) //System.out.println(oldClass.getName() +
				 * " is subsumed by " + newClass.getName()); if (subsumes)
				 * //System.out.println(newClass.getName() + " is subsumed by "
				 * + oldClass.getName());
				 * 
				 * if (subsumed && subsumes) return Matchmaker.EQUIVALENT; if
				 * (subsumed && !subsumes) return Matchmaker.SUBSUMED; if
				 * (!subsumed && subsumes) return Matchmaker.SUBSUMES;
				 */
			} catch (DIGReasonerException e) {
				System.out.println("DIG Exception comparing "
						+ oldType.getName() + " with " + newType.getName()
						+ ": " + e.getMessage());
