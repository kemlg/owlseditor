...
public class MatchingProcessFinder extends MatchFinder {
	...
	/** Creates a new Perform instances with copies of the dataflow in the old one,
	 * in new InputBinding and ValueOf instances. This can then be sent to
	...
	private OWLIndividual copyPerform(OWLIndividual oldPerform){
		...
		while (it.hasNext()){
			...
			OWLIndividual oldToParam = (OWLIndividual)oldInputBinding.getPropertyValue(toParam);
			OWLIndividual oldValueOf = (OWLIndividual)oldInputBinding.getPropertyValue(valueSource);
			OWLIndividual oldFromPerform = (OWLIndividual)oldValueOf.getPropertyValue(fromProcess);
			OWLIndividual oldFromParam = (OWLIndividual)oldValueOf.getPropertyValue(theVar);
	...
	protected void updateIncomingDataFlow(MatchResult mResult, OWLIndividual theperform){
		...
		while (it.hasNext()){
			...
			OWLIndividual targetParam = (OWLIndividual)inputBinding.getPropertyValue(toParam);
			...
			if (pair != null){
				if (pair.getMatchType() != Matchmaker.FAIL){
					...
					// There is no new binding to replace this old one, so we delete it
				...
				System.out.println("ERROR! MatchingProcessFinder.java: Target parameter " + 
									targetParam.getName() + " not accounted for!");
		}
	}
	
	/** Outgoing dataflow from the Perform.
	 * Any InputBinding with this perform as its source must be updated.
	 * If mResult is null, the outgoing dataflow will simply be removed.
	 */
	protected void updateOutgoingDataFlow(MatchResult mResult, OWLIndividual theperform){
			...
			if (valueOf != null){
				...
				if (fromPerform == theperform){
					...
					if (mResult != null){
						...
						//else
						//	System.out.println("No match for " + fromParam.getName());
					}
					else{
	...
	private void addAsChoice(OWLIndividual oldPerform, MatchResult mResult){
		...
		// If the parent isn't already a Choice, we create a new Choice parent
			...
			parentList.removeAtIndex(index, false);   // we keep the perform instance
