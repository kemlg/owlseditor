...
/**
 * This is a static class serving as a central hub for the matchmaking
 * functionality. It lets different MatchProvider modules register themselves,
 * and for each match query, it returns results from all registered modules. For
 * now, we implement a LocalMatchProvider module, which does matchmaking on
 * local processes. But we can extend this to provide UDDIMatchProvider,
 * P2PMatchProvider, etc.
 * 
...
public class Matchmaker implements CleanerListener {
	...
	public List findMatchingProcesses(OWLIndividual perform) {
		...
		OWLIndividual process = (OWLIndividual) perform
				.getPropertyValue(processProperty);

		System.out.println("Matchmaker looking for matching processs for "
				+ process.getName());
	...
	/**
	 * Returns a list of matches that have outputs to match the provided
	 * process's inputs.
	 */
	public List findInputProviders(OWLIndividual perform) {
		...
		System.out.println("Matchmaker looking for input providers for "
				+ process.getName());
	...
	/**
	 * Returns a list of matches that have inputs to match the provided
	 * process's outputs.
	 */
	public List findOutputConsumers(OWLIndividual perform) {
		...
		System.out.println("Matchmaker looking for output consumers for "
				+ process.getName());
	...
	/**
	 * Any input for which there already exists an input binding is removed from
	 * the matches. This is done before scoring, to reflect the fact that a
	 * match is not of much use if we already have a provide for the parameter.
	 * This also means that the user can search for input providers several
	 * times, and get successively more specific results. If new matches are
	 * desired for inputs that are already bound (e.g. the existing match is a
	 * subsume match and an equivalent match is desired) then the user should
	...
	private void removeBoundInputs(MatchResult mResult, OWLIndividual perform) {
		...
		OWLObjectProperty hasInput = model
				.getOWLObjectProperty("process:hasInput");
		OWLObjectProperty hasDataFrom = model
				.getOWLObjectProperty("process:hasDataFrom");
		OWLObjectProperty toParam = model
				.getOWLObjectProperty("process:toParam");

		// System.out.println("removeBoundInputs on " +
		// mResult.getProcessString());

		Iterator it = mResult.getOriginalProcess().getPropertyValues(hasInput)
				.iterator();
		while (it.hasNext()) {
			...
			while (it2.hasNext()) {
				...
				OWLIndividual targetInput = (OWLIndividual) inputBinding
				...
				if (targetInput != null) {
					...
					if (pair.getOriginalParameter() == targetInput) {
						// System.out.println("Removing input match for " +
						// targetInput.getName());
	 ...
	 * declarations.
	 */
	private void removeBoundOutputs(MatchResult mResult,
			OWLIndividual theperform) {
		OWLIndividual originalProcess = mResult.getOriginalProcess();
		OWLModel model = originalProcess.getOWLModel();
		OWLNamedClass bindingCls = model.getOWLNamedClass("process:Binding");
		OWLObjectProperty valueSource = model
				.getOWLObjectProperty("process:valueSource");
		OWLObjectProperty fromProcess = model
				.getOWLObjectProperty("process:fromProcess");
		OWLObjectProperty theVar = model.getOWLObjectProperty("process:theVar");
		OWLObjectProperty hasOutput = model
				.getOWLObjectProperty("process:hasOutput");

		Collection originalOutputs = mResult.getOriginalProcess()
				.getPropertyValues(hasOutput);
		...
		while (it.hasNext()) {
			...
			if (valueOf != null) {
				...
				if (fromPerform == theperform
						&& originalOutputs.contains(fromParam))
	...
	private double calculateInputProviderScore(MatchResult match) {
		...
		int totalParams = perfectInputMatches + subsumesInputMatches
				+ subsumedInputMatches + nonInputMatches;
		...
		double score = (perfectInputMatches
				+ (subsumesInputMatches * HIGH_SUBSUME_FACTOR) + (subsumedInputMatches * LOW_SUBSUME_FACTOR))
	...
	private double calculateOutputConsumerScore(MatchResult match) {
		...
		int totalParams = perfectMatches + subsumesMatches + subsumedMatches
				+ nonMatches;
		...
		double score = (perfectMatches + (subsumesMatches * LOW_SUBSUME_FACTOR) + (subsumedMatches * HIGH_SUBSUME_FACTOR))
	...
	private double calculateMatchingProcessScore(MatchResult match) {
		...
		int totalParams = perfectMatches + goodSubsumeMatches
				+ badSubsumeMatches + nonMatches;
		int extraInputs = match.getExtraInputCount();
		int extraOutputs = match.getExtraOutputCount();
		...
		double score = (perfectMatches
				+ (goodSubsumeMatches * HIGH_SUBSUME_FACTOR) + (badSubsumeMatches * LOW_SUBSUME_FACTOR))
				/ (totalParams + (extraInputs * EXTRA_INPUT_FACTOR) + (extraOutputs * EXTRA_OUTPUT_FACTOR));
