...
public class ExecuteFrame extends JFrame {
	...
	public ExecuteFrame(OWLModel okb, OWLIndividual selectedInst) {
		...
		parameterpanel
				.setLayout(new BoxLayout(parameterpanel, BoxLayout.X_AXIS));
		...
		splitter = new JSplitPane(JSplitPane.VERTICAL_SPLIT, parameterpanel,
				controlpanel);
		pane.add(splitter);
	...
	JScrollPane createInputOutputPanel(boolean inputOroutput) {
		// inputOroutput=true for inputpanel....inputOroutput=false for
		// outputpanel
		...
		while (it.hasNext()) {
			...
			JLabel label1 = new JLabel(x.getparameterName() + ":",
					JLabel.TRAILING);
			...
			if (x.isXSDType() && inputOroutput) {
				if (t.equals("string")) {
					...
					cmp = new JCheckBox();
				} else if (t.equals("decimal") || t.equals("integer")
				...
				if (inputOroutput) { // Instant Chooser for inputs
					...
					String prefix = okb.getNamespaceManager().getPrefix(
							x.getparameterResource());
					String classname = x.getparameterTypeString();
					if (prefix != null)
						classname = prefix + ":" + classname;
					cmp = new OWLSInstanceList(okb.getProject(), classname,
							false);
	...
	void onExecute(ActionEvent e) {
		...
		while (it.hasNext()) {
			...
			if (sTemp.getURI().toString()
					.equals(theService.getURI().toString())) {
				service = sTemp;
				break;
			}
		}
		...
		while (it.hasNext()) {
			...
			if (value instanceof RDFIndividual) {
				values.setValue(p, ont.getIndividual(URI
						.create(((RDFIndividual) value).getURI())));
			} else {
	...
	void OnExecutionEvent(int event) {
		...
		case ExecutionEngine.EXECUTEION_SUCCESSFUL:
			...
			while (it.hasNext()) {
				ProcessParameter x = (ProcessParameter) it.next();
				OWLValue outValue = engine.GetOutput().getValue(
						engine.GetProcess().getOutputs()
								.getParameter(x.getparameterName()));
				/*
				 * if ( outValue instanceof OWLValue ) { reload = true; String
				 * data = ((org.mindswap.owl.OWLIndividual)outValue).toRDF();
				 * File instfile = null; try { instfile =
				 * File.createTempFile("Instance_",".owl",new File("."));
				 * FileOutputStream outstream = new FileOutputStream(instfile);
				 * outstream.write(data.getBytes()); outstream.flush();
				 * outstream.close(); } catch ( Exception e ) {
				 * e.printStackTrace(); continue; }
				 * 
				 * GenerateFromWSDLAction.importOWLFile(instfile.getName(),okb,"in"
				 * ,false); }
				 */
				x.setValue(outValue);
			}
	...
	boolean FetchParametersFromProcess() {
		...
		OWLIndividual theProcess = (OWLIndividual) theService
				.getPropertyValue(processes);
		if (theProcess == null)
			return false;
		...
		while (it2.hasNext()) {
			...
			if (inst.getPropertyValue(paramType) == null)
				return false;
			x.setparameterType(URI.create(inst.getPropertyValue(paramType)
					.toString()));
