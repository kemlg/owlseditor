...
public class Bindings{
	...
	public void addBinding(OWLIndividual fromProcess, OWLIndividual theVar, OWLIndividual toParam){
	...
	private UniqueName getPerformNodeName(String nodename, OWLIndividual theprocess){
		if (theprocess.hasRDFType(atomicProcess, true))
			return new UniqueName(PerformNode.ATOMIC_PERFORM_PREFIX + nodename, new HashSet());
		else if (theprocess.hasRDFType(compositeProcess, true))
			return new UniqueName(PerformNode.COMPOSITE_PERFORM_PREFIX + nodename, new HashSet());
		else if (theprocess.hasRDFType(simpleProcess, true))
			return new UniqueName(PerformNode.SIMPLE_PERFORM_PREFIX + nodename, new HashSet());
	...
	private String getFromNodeName(OWLIndividual fromPerform){
		...
		OWLIndividual fromProcess = (OWLIndividual)fromPerform.getPropertyValue(processProperty);
	...
	public void graphBindings(PrintWriter pw){
		...
		while (it.hasNext()){
			...
			UniqueName dfnode = new UniqueName("DataFlow" + fromProcess.getName(), nameSet);
			...
			pw.print("fillcolor=" + GraphProcessModel.NODE_FILL_COLOR + ", ");   // this is so it doesn't get highlighted
