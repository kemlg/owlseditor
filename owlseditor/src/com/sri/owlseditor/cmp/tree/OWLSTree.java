...
public class OWLSTree extends JTree implements CleanerListener, 
	...
	public OWLSTree(OWLModel okb, DefaultTreeModel treemodel, GraphUpdateManager mgr){
		this.okb = okb;
		this.mgr = mgr;
		setModel(treemodel);
	...
	public void cleanup(){
		...
		instrument.removeListeners();  // make sure this one also removes the vetoable... listeners
		removeListeners();
		//getModel().removeTreeModelListener(this);
		instrument = null;
	}
	...
	/** We override this and call setExpandedState, since expandState
	...
	/** 
	 * This makes sure that the tree is unselected when the user clicks
	 * outside of the tree. What actually happens is that the (invisible)
	 * root is selected, so that any subsequent node creation will take
	 * place under the root.
	 * 
	...
	public void processMouseEvent(MouseEvent ev)
    {
       ...
       // Fixed - I think.
       //
       // But also when root construct is dragged into second construct, and then
	...
	/** Creates a node of a specified type and adds it  as the last child of 
	 * the currently selected node in the tree. Used by the add buttons. 
	 */
	public void addNode(String type){
		...
		if (selectedNode == null || selectedNode.acceptsChild()){
			...
			JOptionPane.showMessageDialog(null,
					"Cannot create " + type + " construct at the selected location.",
	 ...
	 * both the tree objects and the protege objects. 
	...
	public void deleteNode(OWLSTreeNode node){
		...
		for (int i=0; i<c; i++){
			/* We keep deleting child with index 0, which will range
	...
	/** Adds an existing OWLSTreeNode at a specified place in the tree. Called
	 * by both addNode() (manual node creation) and nodeCreation() (dnd node move). */
	public void nodeAdded(OWLSTreeNode node){
		...
		if (((node instanceof PerformNode) || (node instanceof ProduceNode)) 
			...
			PerformTreeMapper.getInstance().generatePredecessors(root, perform, getSelectedProcess());
		}
	...
	protected void removeOutgoingDataflow(OWLIndividual theperform){
		...
		OWLObjectProperty valueSource = okb.getOWLObjectProperty("process:valueSource");
		OWLObjectProperty fromProcess = okb.getOWLObjectProperty("process:fromProcess");
	...
	public void nodeDeleted(OWLSTreeNode node){
		...
		//System.out.println("Deleting node " + node + " with parent " + parent);
	 ...
	 * IJTree methods, and only happen on drag-and-drop. Since there can
	 * only be one drag-and-drop going on at a time, we can use an instance variable
	 * to keep track of the state of the dnd.
	 * 
	...
	public void nodeCreation(VetoableTreeStructureChangeEvent e) throws VetoException{
		...
		//System.out.println("New index: " + index + ", New parent: " + parent);
	...
	public void nodeDeletion(VetoableTreeStructureChangeEvent e) throws VetoException{
		...
		OWLSTreeNode node = (OWLSTreeNode)e.getNode();
		//System.out.println("Old index: " + e.getOldIndex() + ", Old parent: " + e.getOldParent());
		//System.out.println("Node " + node);

		if (vetoCurrentDnd){
			creationPerformed = false;
			vetoCurrentDnd = false;
			throw new VetoException("nodeDeletion() vetoed because node creation was" +
									" vetoed.");
		}
			...
			JOptionPane.showMessageDialog(null,
					...
					"Error moving control construct!",
				    JOptionPane.ERROR_MESSAGE);
			throw new VetoException("nodeDeletion() vetoed because nodes cannot be dragged " +
	...
	/* This is never called. A drag-and-drop node move constists of a nodeCreation
	 * followed by a node deletion.
	 */
	public void nodeMove(VetoableTreeStructureChangeEvent e) throws VetoException{
		//System.out.println("Vetoable node move");
		throw new VetoException("This operation is not supported!");
	}
     
	public void	nodeRename(VetoableTreeStructureChangeEvent e) throws VetoException{
