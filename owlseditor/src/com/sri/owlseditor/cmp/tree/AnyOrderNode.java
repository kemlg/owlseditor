...
public class AnyOrderNode extends BagOrTreeNode {
	...
	public GraphNodeInfo graph(HashSet nameSet, PrintWriter pw, int clusterNumber, 
		...
		pw.println(inNodeName.getUniqueName() + " [shape=point, label=\"Any-Order\"];");

		Enumeration e = children();
		if (!e.hasMoreElements()){
			// nothing inside this construct
			pw.println("}");
	        return new GraphNodeInfo(inNodeName.getUniqueName(), inNodeName.getUniqueName(), 
	        						 "", "", newClusterNumber);
		}

		// Create the out node
		UniqueName outNodeName = new UniqueName("outNode", nameSet);
		pw.println(outNodeName.getUniqueName() + " [shape=point, label=\"/Any-Order\"];");
        ...
        pw.println("subgraph \"" + clusterName + 
        			"\" {label=\"Any-Order\";" +
					" labelloc=top;" +
        			" style=dotted;");
        ...
        // Create the invisible in node
		UniqueName inNodeName = new UniqueName("inNode", nameSet);
		pw.println(inNodeName.getUniqueName() + " [shape=point, label=\"\"];");
		
		// Create the invisible out node
		UniqueName outNodeName = new UniqueName("outNode", nameSet);
		pw.println(outNodeName.getUniqueName() + " [shape=point, label=\"\"];");
		...
		for (Enumeration e = children() ; e.hasMoreElements() ;) {
			...
			thisPair = child.graph(nameSet, pw, newClusterNumber++, selectedNode);
			newClusterNumber = thisPair.clusterNumber;
			// connect new node to the in and out nodes using invisible edges			
			pw.println(inNodeName.getUniqueName() + "->" + thisPair.firstNode +
						thisPair.createInEdgeAttr("style=invis"));			
			pw.println(thisPair.lastNode + "->" + outNodeName.getUniqueName() +
