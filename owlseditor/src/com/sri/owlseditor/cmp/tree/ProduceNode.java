...
public class ProduceNode extends OWLSTreeNode {
	...
	public GraphNodeInfo graph(HashSet nameSet, PrintWriter pw,
		...
		/*
		 * if (getProcess() == null){ // No process for this Perform. Draw it in
		 * red. pw.println("node [color=" +
		 * GraphProcessModel.ERROR_NODE_EDGE_COLOR + ", fontcolor=" +
		 * GraphProcessModel.ERROR_NODE_FONT_COLOR + "];");
		 * pw.println(controlConstructName.getUniqueName()+"[label=\"?\"];"); }
		 * else if (performsCompositeProcess())
		 * pw.println(controlConstructName.getUniqueName
		 * ()+"[peripheries=2, label=\"" + getProcess().getName() +"\"];"); else
		 * // atomic or simple process
		 * pw.println(controlConstructName.getUniqueName()+"[label=\"" +
		 * getProcess().getName() +"\"];");
		 */

		pw.println(controlConstructName.getUniqueName()
				+ "[shape=\"circle\", height=\"0.1\", "
				+ "width=\"0.1\", label=\"P\"];");
		...
		return new GraphNodeInfo(controlConstructName.getUniqueName(),
				controlConstructName.getUniqueName(), "", "", newClusterNumber);
	}

	private void addOutputBindings(PrintWriter pw, HashSet nameSet) {
		Collection outputBindings = getInstance().getPropertyValues(
		...
		while (it.hasNext()) {
			...
			if (valueOf != null) {
				...
				OWLIndividual toParam = (OWLIndividual) outputBinding
	...
	public void deleteBindings() {
		...
		Iterator bindings = produce.getPropertyValues(producedBindingSlot)
				.iterator();
