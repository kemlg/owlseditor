...
public class PerformNode extends OWLSTreeNode {
	...
	public GraphNodeInfo graph(HashSet nameSet, PrintWriter pw, int clusterNumber,
        ...
        if (getProcess() == null){
        	...
        	unodename = new UniqueName(PERFORM_PREFIX + getInstance().getName(), nameSet);
        	nodename = unodename.getUniqueName(); 
        	pw.println(nodename + "[label=\"?\"];");
        }
		else if (performsCompositeProcess()){
        	unodename = new UniqueName(COMPOSITE_PERFORM_PREFIX + getInstance().getName(), nameSet);
        	nodename = unodename.getUniqueName(); 
        	pw.println(nodename + "[peripheries=2, label=\"" +
        			   getProcess().getName() +"\"];");
		}
		else if (performsAtomicProcess()){
        	unodename = new UniqueName(ATOMIC_PERFORM_PREFIX + getInstance().getName(), nameSet);
        	nodename = unodename.getUniqueName(); 
			pw.println(nodename + "[label=\"" +
					   getProcess().getName() +"\"];");
		}
		else{  // simple process
        	unodename = new UniqueName(SIMPLE_PERFORM_PREFIX + getInstance().getName(), nameSet);
        	nodename = unodename.getUniqueName(); 
			pw.println(nodename + "[label=\"" +
    				   getProcess().getName() +"\"];");
		}
        ...
        return new GraphNodeInfo(nodename, nodename, 
								 "", "", newClusterNumber);
	}

	private void addInputBindings(PrintWriter pw, HashSet nameSet){
		
		Collection inputBindings = getInstance().getPropertyValues(hasDataFromSlot, true);
		...
		while (it.hasNext()){
			...
			if (valueOf != null){
				...
				OWLIndividual toParam = (OWLIndividual)inputBinding.getPropertyValue(toParamSlot);
	...
	*/
	...
	public void deleteBindings(){
		...
		while (it.hasNext()){
			...
			System.out.println("Deleting InputBinding " + inputBinding.getName());
	...
	/** For Performs, we print "Perform x", where x is the name of the 
	 * 	process that is performed.
	 * 
	 *  We add some whitespace after the name, to leave room for the process
	 *  type mini-icon (see OWLSTreeCellRenderer).
	 */
	...
	public boolean performsCompositeProcess(){
		...
		if (process == null)
			return false;
		else if (process.hasRDFType(getOWLModel().getOWLNamedClass("process:CompositeProcess")))
			return true;
		else
			return false;
	}
	...
	public boolean performsAtomicProcess(){
		...
		if (process == null)
			return false;
		else if (process.hasRDFType(getOWLModel().getOWLNamedClass("process:AtomicProcess")))
			return true;
		else
			return false;
	}
	...
	public OWLIndividual getProcess(){
		return (OWLIndividual)OWLUtils.getNamedSlotValue(getInstance(), "process:process", getOWLModel());
	}
