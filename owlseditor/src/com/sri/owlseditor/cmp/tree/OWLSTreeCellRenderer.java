...
public class OWLSTreeCellRenderer
   extends DefaultTreeCellRenderer
{
   ...
   public Dimension getPreferredSize(){
   		// We set a very big size here, and then reduce it later, in the paint() method,
   		// since we can't increase the size of the Graphics object given to paint().
   		Dimension size = super.getPreferredSize();
   		if (size.getWidth() < CELL_WIDTH)
   			return new Dimension(CELL_WIDTH,15);
   		return size;
   }
   */
      ...
      boolean hasFocus)
   {
      if (value instanceof OWLSTreeNode
         && ((OWLSTreeNode)value).getUserObject()
            instanceof DragOverData)
      {
        m_node = (OWLSTreeNode)value;
        m_nDndEffect =
            ((DragOverData)
            m_node.getUserObject()).getCurrentEffect();
      }
      else
    ...
    * drag-over effect is "drop above", paints a horizontal black line
    * across the top of the component. If the effect is "drop below", paints
    * a horizontal black line across the bottom of the component. If the
    * effect is "drop into (as child)", paints a vertical black line along
    * the left side of the component. All these lines are two pixels wide
    * and not particularly pretty.
    */
   public void paint(Graphics g)
   {
   /*
   		JLabel dummy = new JLabel(m_node.toString());
   	  Dimension size = dummy.getPreferredSize();
   	  System.out.println("Node label is " + dummy.getText() + ", preferrred size is " + size);
   	  Dimension newsize = new Dimension();
   	  if (m_node instanceof PerformNode)	// Performs need room for the mini-icons
   	  	newsize.setSize(size.getWidth()+getLeafIcon().getIconWidth()+20, size.getHeight());
   	  else
   	  	newsize.setSize(size.getWidth()+getOpenIcon().getIconWidth()+5, size.getHeight());
   	  setSize(new Dimension(newsize));
*/
   	  super.paint(g);
    	...
    	if (inst != null){
    		...
    		while (it.hasNext()){
    			...
    			if (type.getName().equals("process:AtomicProcess")){
    				paintIcon(g,
    						  OWLSIcons.getAtomicProcessIcon(),
							  new Point(iconPos, 0),
							  new Dimension(15, 16));
    				break;
    			}
    			else if (type.getName().equals("process:CompositeProcess")){
    				paintIcon(g,
    		    			  OWLSIcons.getCompositeProcessIcon(),
							  new Point(iconPos, 0),
							  new Dimension(15, 16));
    				break;
    				...
    				paintIcon(g,
  		    			  	  OWLSIcons.getSimpleProcessIcon(),
							  new Point(iconPos, 0),
							  new Dimension(15, 16));
    				break;
    			}
