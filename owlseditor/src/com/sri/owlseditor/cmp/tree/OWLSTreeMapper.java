 ...
 * 
 * This class implements the CleanerListener interface to remove all TreeModelListeners
 * from all its TreeModels when the thing shuts down, and to create a new instance
 * for the new KB (see below).
 * 
 */
public class OWLSTreeMapper implements CleanerListener {
	...
	public static OWLSTreeMapper getInstance(OWLModel okb){
		if (instance == null){
			...
			if (instance.okb != okb){
				System.out.println("ERROR! This OWLSTreeMapper was created for a different" +
									" KB " + instance.okb + ", and is now being used with " + okb);
				return null;
			}
			else
				return instance;
	 ...
	 * and reset the instance pointer, so that a new Mapper is created for the new KB. */
	public void cleanup(){
	...
	/** Returns the OWLSTree associated with a given composite
	 * process. If there is no tree, one will be created
	 * (that is the whole beauty of this class).
	 * @param process
	 * @param listener
	 * @return
	 */
	public OWLSTree getTree(OWLIndividual process, GraphUpdateManager mgr){
		OWLSTree tree = (OWLSTree)process2tree.get(process);
		if (tree == null){
			/* We have no model for this process, so we create one
			 * and add a mapping for it */
			//System.out.println("OWLSTreeMapper.getTree, creating tree for " + process.getName());
			...
			if (extraConstructs != null){
				// There are extra constructs stored in the project file, so we have
	...
	/** Returns the composite process Instance associated with a
	 * given OWLSTree.
	 * @param tree
	 * @return
	 */
	...
	/** Creates a complete OWLSTree for a composite process
	 * instance
	 * @param inst
	 * @return
	 */
	...
	/** Creates the tree for a composite process instance, and returns the root node
	 *  of this tree. */
	...
	/* Returns the process:components value for a Control Construct. If it is null, it will
	 * be set to list:nil, and this will be returned.
	 */ 
	private OWLIndividual getComponents(OWLIndividual inst){
		OWLIndividual components = (OWLIndividual) OWLUtils.getNamedSlotValue(inst, "process:components", okb);
		if (components == null){
	...
	/* This is just a record type return value for the createTree method.
	 * We need to return both the OWLSTreeNode created, and the Collection of
	 * predecesors.
	 */
	/*
	class CreateInfo{
		private OWLSTreeNode node;
		private Vector preds;
		
		CreateInfo(OWLSTreeNode node, Vector preds){
			this.node = node;
			this.preds = preds;
		}
		
		OWLSTreeNode getNode(){
			return node;
		}
	...
	*/
	...
	/** 
	 * Creates the tree from a ControlConstruct. It returns the root node created even
	 * though the caller already has it, since the method is recursive, and this is needed 
	 * internally. This method is only called when the user switches to a different composite
	 * process, to recreate the whole tree.
	 * 
	 * Note: This method is not used to create RootNode instances - those are created directly
	 * (i.e. using new RootNode()).
	 * 
	 * We also generate Vectors of unconditional predecessor Performs, and associate those 
	 * with the Perform constructs using the PerformTreeMapper class. See that class for more info.
	 * 
	 * */
	private OWLSTreeNode createTree(OWLSTreeNodeInfo ni, OWLIndividual cp, Set preds){
		...
		// This may need to be handled better (marking the nodes somehow).
			...
			OWLIndividual whileProcess = (OWLIndividual) OWLUtils.getNamedSlotValue(inst, "process:whileProcess", okb);
			OWLSTreeNodeInfo newni = new OWLSTreeNodeInfo(whileProcess, okb);
			HashSet mypreds = new HashSet(preds);
			newnode.add(createTree(newni, cp, mypreds));
		}	
		else if (newnode instanceof RepeatUntilNode){
			OWLIndividual untilProcess = (OWLIndividual) OWLUtils.getNamedSlotValue(inst, "process:untilProcess", okb);
