...
/**
 * This class (and its subclasses) handle the UI of OWL-S tree nodes, as
 * well as the Protege KB manipulation that needs to occur on node
 * manipulation. 
 * 
...
public abstract class OWLSTreeNode extends DefaultMutableTreeNode {
	...
	public abstract GraphNodeInfo graph(HashSet nameSet, 
								   PrintWriter pw, 
								   int clusterNumber,
								   OWLSTreeNode selectedNode);
	 
	/** Subclasses of OWLSTreeNode must implement their own logic to
	 * handle the synchronization with the protege kb after node deletion.
	 * @param node
	 */
	public abstract void updateKBAfterDelete(OWLSTreeNode node); 

	/** Subclasses of OWLSTreeNode must implement their own logic to
	 * handle the synchronization with the protege kb after moving nodes
	 * (i.e. drag-and-drop).
	 * @param node
	 */
	...
	/** Subclasses of OWLSTreeNode must implement their own logic to
	 * handle the synchronization with the protege kb after node insertion.
	 * @param node
	 */
	public abstract void updateKBAfterInsert(OWLSTreeNode newnode);
	
	/** Subclasses of OWLSTreeNode must implement this method
	 * according to their specific rules for when to accept a new
	 * child. This is used when the user clicks the create buttons
	 * or drags a node on top of this one.
	 * @param o
	 * @return
	 */
