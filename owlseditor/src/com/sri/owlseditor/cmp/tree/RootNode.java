...
/**
 * This class represents the root of the control flow tree of an OWL-S composite
 * process. It has no direct correlate in the OWL-S ontology, but is necessary
 * for the GUI, and to perform some Protege bookkeeping.
 * 
 * OWL-S processes can only have one root control construct, i.e. the value of
 * the process:composedOf property. But we allow users to create several
 * constructs in the GUI. Only the first one will be used as the value of
 * composedOf, but it can be useful to be able to create several constructs, and
 * drag and drop around until satisfaction occurs.
 * 
 * These "secondary control constructs" will be generated in the protege KB, of
 * course, and they will be associated with the current composite process by
 * saving these mappings in the Protege pprj file.
 * 
 * We should probably make the non-primary constructs show up in a different
 * color or something.
 * 
...
public class RootNode extends OWLSTreeNode {
	...
	private OWLIndividual cp; // the composite process that this node is the
								// root of
	...
	/*
	 * Adds a mapping in the project file from the composite process to a
	 * control construct.
	 */
	private void addMapping(OWLIndividual construct) {
		 ...
		 * construct.getProject().setClientInformation(generateKey(cp),
		 * extraConstructs);
		 * 
		 * System.out.println("Mappings are now:"); Object extras =
		 * construct.getProject().getClientInformation(generateKey(cp)); Vector
		 * v = (Vector)extras; for (Enumeration e = v.elements() ;
		 * e.hasMoreElements() ;) {
	...
	/*
	 * Removes a mapping in the project file from the composite process to a
	 * control construct.
	 */
	private void removeMapping(OWLIndividual construct) {
		/*
		 * System.out.println("RootNode: Removing mapping for " + construct);
		 * extraConstructs.remove(construct); // not sure if this is the way to
		 * remove these things
		 * construct.getProject().setClientInformation(generateKey(cp),
		 * extraConstructs);
		 * 
	...
	private void updateCompositeProcess(OWLIndividual construct) {
		...
		if (construct != null) {
			OWLUtils.setNamedSlotValue(cp, "process:composedOf", construct,
					getOWLModel());
		} else {
			OWLUtils.removeNamedSlotValue(cp, "process:composedOf",
					getOWLModel());
		}
	}
