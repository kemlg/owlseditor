...
/**
 * This is a common abstract superclass for all control constructs that have a process:components
 * property, with a ControlConstructBag or ControlConstructList value.
 * 
...
public abstract class BagOrTreeNode extends OWLSTreeNode {
	...
	public void updateKBAfterDelete(OWLSTreeNode node){
		...
		//OWLIndividual newcell = olist.removeAtIndex(node.getInstance(), index, true);
		//if (index == 0){ 
	...
	public void updateKBAfterMove(OWLSTreeNode node){
		...
		//OWLSList olist = new OWLSList(list, type, getOWLModel());
		//OWLIndividual newcell = olist.removeAtIndex(node.getInstance(), index, false);
		//if (index == 0){ 
		//	getInstance().setPropertyValue(components, newcell);
		//}
	}

	/* Returns the ControlConstructBag or -List containing the components of this control
	 * construct, or creates a new one if there is none. 
	 */
	protected OWLIndividual getComponents(){
		OWLModel model = getOWLModel();
		OWLIndividual bagInstance = getInstance();
		OWLIndividual bag = (OWLIndividual)bagInstance.getPropertyValue(components);
	...
	public void updateKBAfterInsert(OWLSTreeNode newnode){
		/* Now we need to put the Instance in the correct
		 place in its ControlConstructBag */
		//OWLIndividual list = getComponents();			
		int newindex = getIndex(newnode);
		OWLSList olist = new OWLSList(getInstance(), getOWLModel());
		olist.insertAtIndex(newnode.getInstance(), newindex);
		//OWLSList olist = new OWLSList(list, type, getOWLModel());
		//OWLIndividual newcell = olist.insertAtIndex(newnode.getInstance(), newindex); 
		/* If we are inserting at the beginning of the bag/list, we
		 * need to update the control construct.
		 */
		//if (newindex == 0){ 
		//	getInstance().setPropertyValue(components, newcell);
		//}
