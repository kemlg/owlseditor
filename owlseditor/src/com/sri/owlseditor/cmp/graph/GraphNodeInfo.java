...
/**
 * This class is used for the return value of the graph() method in OWLSTreeNode.
 * When creating a sub-graph, we must return the incoming and outgoing connection points
 * of the sub-graph, as well as attributes that we want on the in- and outgoing edges, 
 * so that the caller can connect the new subgraph correctly. 
 * 
 * The first- and lastNode can be the same (this happens for Split, for example).
 * 
...
public class GraphNodeInfo {
	...
	public GraphNodeInfo(String firstNode, String lastNode, 
						 String inEdgeAttr, String outEdgeAttr,
						 int clusterNumber){
	 ...
	 * If there are no resulting attributes, an empty string is returned.
	 * 
	 * @param attr comma-separated list of edge attributes from the DOT language.
	 * @return
	 */ 
