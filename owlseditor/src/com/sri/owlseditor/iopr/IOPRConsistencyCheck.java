...
public class IOPRConsistencyCheck {
    ...
    public static IOPRConsistencyCheck getInstance (OWLModel okb, IOPRSelector ioprList) {
	if (instance == null)
	...
	Collection profileInsts = _okb.getOWLNamedClass("profile:Profile").getInstances (true);
	...
	if (profileInsts != null) {
		...
		OWLIndividual processInst = (OWLIndividual) profileInst.getPropertyValue(has_process);
		
		if (processInst != null){
			Collection ioprProperties = m_ioprList.getIOPRproperties();
			Iterator i2 = ioprProperties.iterator();
	
			while (i2.hasNext()) {  // iterate over ioprProperties (hasInput, hasOutput, etc)
			    String ioprProperty = (String) i2.next();
			    Collection profileIOPRs = profileInst.getPropertyValues (_okb.getOWLProperty ("profile" + ioprProperty), true);
			    Collection processIOPRs = processInst.getPropertyValues (_okb.getOWLProperty ("process" + ioprProperty), true);
			    Iterator itt = profileIOPRs.iterator();
	
			    while (itt.hasNext()) { // iterate over listitems that have profile checked
				RDFIndividual profileIOPR = (RDFIndividual) itt.next();
				boolean isMatch = checkForMatch (profileIOPR, processIOPRs);
				if (!isMatch) {
				    createInconsistencyEntry1 (profileIOPR, profileInst, processInst, ioprProperty);
				}
			    }
			}
		}
	...
	OWLDatatypeProperty paramType = _okb.getOWLDatatypeProperty ("process:parameterType");

	while (it.hasNext()) {
	    RDFResource rdfr = (RDFResource) it.next();
	    RDFSLiteral literal = (RDFSLiteral) rdfr.getPropertyValue (paramType);
	    ...
	    if (!(resource instanceof RDFSClass) && !(resource instanceof RDFSDatatype)) {
    ...
    private void createInconsistencyEntry1 (RDFIndividual inst, OWLIndividual profile, OWLIndividual process, String ioprProperty) {
	// label object followed by problem object followed by vector of n solution objects
    ...
    private boolean checkForMatch (RDFIndividual profileVal, Collection processVals) {
	...
	else  // must be profile, so remove profile
	...
	DefaultRDFSLiteral literal = (DefaultRDFSLiteral) _okb.createRDFSLiteral (onc.getURI(), 
			 _okb.getRDFSDatatypeByURI(XSDDatatype.XSDanyURI.getURI()));
