...
/**
 * A panel with two rows of checkboxes. Used by the OWLSDirectInstancesList class.
 * 
 * This class maintains a mapping from the instances in the OWLSDirectInstancesList and
 * the corresponding checkboxes. Adding/deleting instances add/deletes a pair of checkboxes
 * and updates the mapping.
 * 
 * The action handlers for the checkboxes, modifying the KB, are taken care of by this class itself.
 * 
...
public class ListAndCheckboxPanel extends JPanel implements ComboListener {
	...
	jcb1.addMouseListener (new MouseAdapter() {
		public void mouseClicked (MouseEvent me) {
		    ...
		    if (selectedIndex < 0)
			return;
		    JCheckBox cb = (JCheckBox) jcb1.getModel().getElementAt (selectedIndex);
	...
	jcb2.addMouseListener (new MouseAdapter() {
		public void mouseClicked (MouseEvent me) {
		    ...
		    if (selectedIndex < 0)
			return;
		    JCheckBox cb = (JCheckBox) jcb2.getModel().getElementAt (selectedIndex);
		    cb.setSelected (!cb.isSelected());
		    jcb2.repaint();
		}
	    });
	...
	for (int i = 0; i < slm.getSize(); i++) {
	    ((JCheckBox) dlm1.getElementAt(i)).setEnabled (false);
	    ((JCheckBox) dlm2.getElementAt(i)).setEnabled (false);
	}
	*/
    }
    ...
    private void addRemovePropertyValue (OWLIndividual owlInst, JCheckBox jcb, DefaultListModel dlm, DefaultListModel dlmx) {
    ...
    public void copyPropertyValue (OWLIndividual leftInst, OWLIndividual rightInst, boolean leftToRight) {
	/*
	String leftPropertyName = getPropertyName ((OWLIndividual) leftInst);
	if (leftPropertyName == null)
	    return;
		
	String rightPropertyName = getPropertyName ((OWLIndividual) rightInst);
	if (rightPropertyName == null)
	    return;
	*/
	//RDFProperty rdfp = (RDFProperty) model.getOWLProperty (leftPropertyName);
	SimpleListModel slm = (SimpleListModel) list.getModel();
	for (int i = 0; i < slm.getSize(); i++) {
	    if (leftToRight) {
	...
	Collection c = inst.getPropertyValues (model.getOWLProperty (propertyName), true);
    ...
    /*
    private int getCBIndx (JCheckBox jcb, DefaultListModel dlm) {
	for (int i = 0; i < dlm.getSize(); i++) {
	    JCheckBox jcbx = (JCheckBox) dlm.getElementAt(i);
	    if (jcbx == jcb)
		return i;
	}
    ...
    */
	...
	for (int i = 0; i < slm.getSize(); i++) {
	    if (inst.getName().equals (((RDFIndividual) slm.getElementAt(i)).getName()))
	...
	public Component getListCellRendererComponent (JList list, Object obj, int indx, boolean isChecked, boolean hasFocus) {
