...
public class W2OXSLTDialog extends XSLTDialog {
	...
	private JButton createClass, createData, createVariable, createRDFAttribute, 
					createDatatypeProp, createObjectProp, deleteNode;
	
	public W2OXSLTDialog(OWLModel okb, OWLIndividual aP, String wsdl, String wsdlMsg, OWLIndividual msgMap) 
		...
		if (wsdlOutputMessage == null)
			throw new XSLTException("WSDL Output Message " + wsdlMsg + " not found.");
	...
	protected void setupGUI() {
		...
		JLabel targetOWLTypeLabel = ComponentFactory.createLabel("Target OWL Type: ");
		...
		JLabel sourceWsdlTypesLabel = ComponentFactory.createLabel("Source WSDL types");
		JTextArea sourceWsdlTypesArea = ComponentFactory.createTextArea();
		sourceWsdlTypesArea.setEditable(false);
		JScrollPane sourceWsdlTypesPane = ComponentFactory.createScrollPane(sourceWsdlTypesArea);
	...
	private OWLIndividual getOwlsParameter(OWLIndividual msgMap){
		OWLObjectProperty owlsParameter = okb.getOWLObjectProperty("grounding:owlsParameter");
	...
	protected void createTranslationPanel() {
	    if ( transPanel == null ) transPanel = new W2OParameterPanel(this);
	}
	...
	protected void addNewNode(XSLTNode n) {
		...
		if (n instanceof VariableNode){
			((W2OParameterPanel)transPanel).getDataPanel().addParameter("$" + n.getNodeName());
			((W2OParameterPanel)transPanel).getRDFNamePanel().addParameter("$" + n.getNodeName());
			((W2OParameterPanel)transPanel).getRDFResourcePanel().addParameter("$" + n.getNodeName());
			((W2OParameterPanel)transPanel).getVariablePanel().addParameter("$" + n.getNodeName());
	...
	private void getUsedOWLNamespaces(Collection namespaces, XSLTNode node){
	    ...
	    if (node instanceof RootNode || 
	    	node instanceof DatatypePropertyNode || 
	...
	private String generateRDFHeader(){
		...
		namespaces.add("xmlns:rdf=\"http://www.w3.org/1999/02/22-rdf-syntax-ns#\"");
	...
	private OWLNamedClass chooseClass(){
		return (OWLNamedClass)DisplayUtilities.
				pickCls((Component)this, 
						okb, 
	...
	private OWLObjectProperty chooseObjectProperty(){
		return (OWLObjectProperty)DisplayUtilities.
				pickInstanceFromCollection((Component)this, 
						OWLUtils.sortFrameCollection(okb.getOWLObjectPropertyClass().getInstances(true)),
						0,
