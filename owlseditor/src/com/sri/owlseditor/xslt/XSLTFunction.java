...
public class XSLTFunction extends Object {
	...
	public XSLTFunction(XSLTFunction f) {
		...
		param = new ArrayList();
		for (int i=0;i<f.getParameterNumbers();i++) param.add(f.getParameter(i));
	}
	public XSLTFunction(String n, int minparam, int maxparam) {
		...
		maxParam = maxparam;
		param = new ArrayList();
		for (int i=0;i<minParam;i++) param.add("");
	}
	...
	public void removeLastParameter() {
		if ( param.size() > 0 ) param.remove(param.size()-1);
	}	
	...
	public void setParameter(int par, String value) {
		if ( par >= 0 && par < maxParam ) {
			while ( param.size() <= par ) param.add("");
			param.set(par,value);
		}
	}
	public String getParameter(int par) {
		if ( par >= 0 && par < param.size() ) return param.get(par).toString();
		return "";
	}	
	public String function2String() {
		...
		if ( temp.equals("") ) return getParameter(0); // No Function, return the first parameter.
		temp += "(";
