...
/**
 * Used for storing the user objects of a complete subtree. Useful in
 * connection with caching, copying and remembering undo information.
 * Very much a utility class.
 */
   ...
   implements Serializable
{
   ...
   public Object getContents() { return m_contents; }
   void setContents(Object ob) { m_contents = ob; }

   /**
    * Creates the subtree. Stores away all user objects in the nodes that
    * are copied.
    *
    * @param node Node to copy
    * @param include_subtree If true, will copy the node and all its
    * children. If false, will only copy the node itself.
    */
   public SubTreeNode(
      DefaultMutableTreeNode node,
      boolean include_subtree)
   {
   	//System.out.println("SubTreeNode() with node " + node + ", include_subtree " + include_subtree);
   ...
   /**
    * Recreates this node and its entire subtree, if any. If the
    * original contents of the copied tree nodes implements Copyable,
    * will make a copy of the contents objects for the recreated tree.
    * If not, the new tree will contain the same contents references
    * as the original.
    *
   ...
   public DefaultMutableTreeNode createNode()
   {
      ...
      if (false){   // we don't want to make a copy, we'll keep the original
      	System.out.println("SubTreeNode.createNode(): Making a copy of node contents.");
      ...
      // this class
      //System.out.println("SubTreeNode.createNode(): new_contents is of " + new_contents.getClass());
      // new_contents is a DefaultNodeInfo, not an OWLSTreeNodeInfo. Not good.      
      OWLSTreeNodeInfo ni = (OWLSTreeNodeInfo)new_contents;  // this is the Instance
