...

{
		...
		if (bb.contains(pt)) {
			...
			while (enum1.hasMoreElements()) {
				if ((subelem = findContainingElement(
						(Subgraph) (enum1.nextElement()), pt)) != null) {
					elem = subelem;
				}
			}
	...
	public static Vector findContainedElements(Subgraph subg, GrappaBox box) {
		...
		if (box.contains(bb)) {
			...
			enum1 = subg.subgraphElements();
			while (enum1.hasMoreElements()) {
				if ((subelems = findContainedElements(
						(Subgraph) (enum1.nextElement()), box)) != null) {
					if (elems == null) {
						elems = new Vector();
					}
					elems.add(subelems);
	...
	public static boolean centerPanel(Point2D cpt, GrappaPanel gpanel) {
		if (cpt == null || gpanel == null)
			return false;
		if (!gpanel.getSubgraph().getBoundingBox()
				.contains(cpt.getX(), cpt.getY())) {
			return false;
		}
