...
public class Subgraph extends Element implements Comparator {
	...
	public java.awt.geom.Rectangle2D getBoundingBox() {
		...
		if (grappaNexus == null || (bbox = grappaNexus.bbox) == null) {
			...
			bbox.add(bbox.getX() + bbox.getWidth() + 1,
					bbox.getY() + bbox.getHeight() + 1);
	...
	private Element findElementInSubgraphByName(int type, String name) {
		...
		while (enum1.hasMoreElements()) {
			if ((elem = ((Subgraph) enum1.nextElement())
	...
	public int countOfElements(int types) {
		...
		if (graphdict != null) {
			...
			Enumeration enum1 = graphdict.elements();
			while (enum1.hasMoreElements()) {
				count += ((Subgraph) enum1.nextElement())
						.countOfElements(types);
			}
		}
		return count;
	}
				...
				if (sg != this) {
					...
					if (attr != null) {
						...
						sg.setAttribute(STYLE_ATTR,
								style != null && style.length() > 0 ? style
										+ ",filled(false)" : null);
					}
				}
			}
			...
			while (enum1.hasMoreElements()) {
				...
				if (sg != this) {
					...
					if (attr == null) {
						...
						style = attr.getStringValue();
						sg.setAttribute(STYLE_ATTR,
								style == null || style.length() == 0 ? "filled"
										: style + ",filled");
					}
				}
			}
				...
				if ((i + 1) < sgPatches.length) {
					...
					for (;;) {
						...
						if (dir > 1) {
							...
							nscore = score(tfrac * box.getWidth(),
									box.getHeight() * sz / tot);
						}
						if (nscore <= pscore) {
							if (dir > 1) {
								...
								pscore = score(tfrac * box.getWidth(),
										box.getHeight() * tsz / tot);
							}
							psz += sz;
							sz = tsz;
							tsz = 0;
			...
			while (i < elPatches.length) {
				...
				if ((i + 1) < elPatches.length) {
					...
					for (;;) {
						...
						for (; i < j; i++) {
							...
							if (el instanceof Node) {
								...
								el.setAttribute(
										POS_ATTR,
										new GrappaPoint(p.getCenterX(), -p
												.getCenterY()));
								el.setAttribute(WIDTH_ATTR,
										new Double(p.getWidth() / 72.0));
								el.setAttribute(HEIGHT_ATTR,
										new Double(p.getHeight() / 72.0));
					...
					if (el instanceof Node) {
						...
						el.setAttribute(
								POS_ATTR,
								new GrappaPoint(p.getCenterX(), -p.getCenterY()));
						...
						if (dir > 1) {
							p = new GrappaBox(box.getX(), previous,
									box.getWidth(), (next = frac
											* box.getHeight()));
						} else {
			...
			for (int i = 0; i < sgPatches.length; i++) {
				...
				if (dir > 1) {
					((Subgraph) el).compStdPatchWork(
							new GrappaBox(box.getX(), previous, box.getWidth(),
									(next = frac * box.getHeight())), false);
				} else {
					((Subgraph) el).compStdPatchWork(
							new GrappaBox(previous, box.getY(), (next = frac
									* box.getWidth()), box.getHeight()), false);
				}
				previous += next;
			}
		}
			...
			for (int i = 0; i < elPatches.length; i++) {
				...
				if (el instanceof Node) {
					...
					if (dir > 1) {
						...
						((Subgraph) el).compStdPatchWork(
								new GrappaBox(previous, box.getY(),
	...
	class Enumerator implements GraphEnumeration {
		...
		Enumerator(int t) {
			...
			if (enum1.hasMoreElements()) {
				dictType = SUBGRAPH;
				while (enum1.hasMoreElements()) {
					subEnum = ((Subgraph) (enum1.nextElement())).new Enumerator(
							types);
			...
			if (enum1 == null) {
				if ((types & NODE) != 0
						&& (enum1 = nodeElements()).hasMoreElements()) {
					dictType = NODE;
					if (elem == null) {
						elem = (Element) enum1.nextElement();
					}
				} else if ((types & EDGE) != 0
						&& (enum1 = edgeElements()).hasMoreElements()) {
					dictType = EDGE;
					if (elem == null) {
						elem = (Element) enum1.nextElement();
					}
