...
public class GrappaNexus implements GrappaConstants, Cloneable, ImageObserver,
	...
	public void updateShape() {
		...
		case SUBGRAPH:
			...
			if (minSize != null) {
				if (sgbox == null) {
					sgbox = new java.awt.geom.Rectangle2D.Double(0, 0,
							minSize.getWidth(), minSize.getHeight());
				} else {
					sgbox.add(new java.awt.geom.Rectangle2D.Double(sgbox
							.getCenterX() - (minSize.getWidth() / 2.0), sgbox
							.getCenterY() - (minSize.getHeight() / 2.0),
							minSize.getWidth(), minSize.getHeight()));
				}
			}
		...
		if (labelAttr != null && labelAttr.length() > 0) {
			...
			if (labelAttr.length() > 0) {
				...
				if (element.isNode()
					...
					if (element.isEdge())
						System.out.println("LP_ATTR = "
								+ element.getLocalAttribute(LP_ATTR));
				...
				for (int l = 0; l < labels.length; l++) {
					if (labels[l] != null && lps[l] != null
						...
						for (i = 0; i < bnds.length; i++) {
							...
							bnds[i].setRect(x, top, bnds[i].getWidth(),
									bnds[i].getHeight());
							if (area == null) {
