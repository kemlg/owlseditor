 ...
 * 
 * @version 1.2, 18 Nov 2004; Copyright 1996 - 2004 by AT&T Corp.
...
public class GrappaPanel extends javax.swing.JPanel implements
	...
	public int print(Graphics g, PageFormat pf, int pi) throws PrinterException {
		...
		try {
			scaleToFit = false;
			scaleToSize = new GrappaSize(pf.getImageableWidth(),
	...
	private Point2D componentPaint(Graphics g) {
		...
		if (scaleToFit || scaleToSize != null) {
			...
			setSize(new Dimension((int) Math.ceil(scaleToWidth),
					(int) Math.ceil(scaleToHeight)));
			setPreferredSize(new Dimension((int) Math.ceil(scaleToWidth),
					(int) Math.ceil(scaleToHeight)));
			...
			if (zoomBox.width != 0 && zoomBox.height != 0
					&& oldTransform != null) {
				GrappaBox zb = new GrappaBox(oldTransform
						.createTransformedShape(zoomBox).getBounds2D());
				...
				setPreferredSize(new Dimension((int) Math.ceil(scaleToWidth),
						(int) Math.ceil(scaleToHeight)));
				transform.translate(-bbox.getMinX(), -bbox.getMinY());
				cpt = new Point2D.Double(zoomBox.getCenterX(),
						zoomBox.getCenterY());
			}
			zoomBox = null;
			...
			scaleInfo = 1;
			setSize(new Dimension((int) Math.ceil(bbox.getWidth()),
					(int) Math.ceil(bbox.getHeight())));
			setPreferredSize(new Dimension((int) Math.ceil(bbox.getWidth()),
					(int) Math.ceil(bbox.getHeight())));
			transform.translate(-bbox.getMinX(), -bbox.getMinY());
	...
	public String getToolTipText(MouseEvent mev) {
		...
		return (grappaListener.grappaTip(subgraph,
				findContainingElement(subgraph, pt), new GrappaPoint(pt.getX(),
						pt.getY()), mev.getModifiers(), this));
	}
	...
	public void setScaleToSize(Dimension2D scaleSize) {
		...
		if (scaleSize == null) {
			...
			scaleToSize = new GrappaSize(scaleSize.getWidth(),
					scaleSize.getHeight());
		}
	}
		...
		if (bb.contains(pt)) {
			...
			while (enum1.hasMoreElements()) {
				if ((subelem = reallyFindContainingElement(
						(Subgraph) (enum1.nextElement()), pt, stash)) != null
						&& subelem.selectable) {
					if (stash[0] == null)
						return (subelem);
					if (stash[1] == null)
		...
		grappaListener
				.grappaClicked(
						...
						new GrappaPoint(pt.getX(), pt.getY()), mev
								.getModifiers(), mev.getClickCount(), this);
	}
	...
	public void mousePressed(MouseEvent mev) {
		...
		grappaListener.grappaPressed(subgraph,
				(pressedElement = findContainingElement(subgraph, pt)),
	...
	public void mouseReleased(MouseEvent mev) {
		...
		grappaListener.grappaReleased(subgraph,
				findContainingElement(subgraph, pt), gpt, modifiers,
				pressedElement, pressedPoint, pressedModifiers, outline, this);
		...
		if ((modifiers & java.awt.event.InputEvent.BUTTON1_MASK) != 0
			...
			if (outline != null) {
				...
				savedOutline = GrappaSupport.boxFromCorners(outline,
						pressedPoint.x, pressedPoint.y, gpt.x, gpt.y);
				outline = null;
			} else {
